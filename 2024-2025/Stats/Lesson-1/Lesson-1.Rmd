---
title: "**Applied statistics 2024-2025: Lesson 1**"
...
The above is for one donor. The scheme with the four donors is shown in Figure \@ref(fig:exp-design-luis-four-donors-mean); the rightmost columns shows one possible way of indexing the observations and the average of the three replicates; that is not relevant now, but will be handy later.
...
(Note: the graph below cannot be created directly from R Commander, but we will create similar ones with R Commander later. You can, of course, copy the code in the console and run it)
...
Now, observe the boxplot below and try to see if you can understand better what was observed in the bar graphs with the "Glucose" and TGN.Glucose" groups (as before, this plot cannot be created directly using R Commander):
...
```{r load-hh,echo=FALSE}
library(HH)
```

Now, you will see that there are more entries under some menus. Go to **Graphs** and click, there **Lattice plots**. We can produce a plot that, except for the error bars, is equivalent to the former graph barchart by selecting these options:

```{r hh-bar-1, fig.cap =  "HH: options for the bar graph.", fig.align= "center", out.width="50%", echo=FALSE}
...
And that produces this code and gives this figure
```{r hh-bar-1-out, fig.cap = "Barchart + lattice using HH"}
...
What about boxplots? We can obtain similar plots. For example, I first selected the following options

```{r hh-bp-1, fig.cap =  "HH: options for the box plot.", fig.align= "center", out.width="50%", echo=FALSE}
knitr::include_graphics("hh-bp-1.png")
```

\clearpage

and those gave this plot (actually, I added the "rot" thing at the end, to have all tick labels horizontal)

```{r hh-bp-1-out, fig.cap =  "Lattice boxplot, rotated y-axis tick labels"}
...
but, to make it comparable to the box plot above, I then edited the command line manually, and transposed "Group" and "Area", and then clicked on "Submit"


```{r hh-bp-2, fig.cap =  "Lattice boxplot: transposed Group and Area"}
...
**Do you think it is always best to allow the scales to differ?** What patterns are highlighted or masked by doing that? Compare with this figure (identical to above, except we did not allow the scales to vary between panels ---shown as "Y scales in different panels" in the GUI and as the "relation = 'same'" in the command)

```{r hh-bp-3, fig.cap =  "Lattice boxplot: same scales for all panels."}
bwplot(ADC ~ Time | Group + Area, outer=TRUE, layout=c(4, 2),
data= Brain_MR, type="p", par.settings=simpleTheme(pch=16),
auto.key=list(border=TRUE),
scales=list(x=list(relation='same', log=FALSE),
...
It is also possible to create plots using ggplot2 with R Commander (again, ggplot2 is the package used to create Figure \@ref(fig:figure6)). To do this we need to install package \texttt{RcmdrPlugin.KMggplot2} (yes, we did not ask you to do this initially because ... we only realized this now). To install the package, from the R console do this

```{r install-kmggplot2, eval=FALSE}
install.packages("RcmdrPlugin.KMggplot2")
```
```{r kmggplot2-load, include=FALSE}
...
In the **KMggplot2** entry, I selected **Box plot/Violin plot/Confidence interval...**. And there, I used the options shown in Figure \@ref(fig:km0-settings) following:
```{r km0-settings, fig.cap =  "KMggplot2: options for the box plot example.", fig.align= "center", out.width="80%", echo=FALSE}
knitr::include_graphics("bp-ggp-0.png")
```

And that produced Figure \@ref(fig:km-bp0) (notice that, here, we cannot allow different scales for each panel, or "free scales")

```{r km-bp0, fig.cap =  "KMggplot2: box plot example.", fig.align= "center", out.width="100%", echo=TRUE}
...
To show some additional options, like coloring of each box plot and a different kind of plot, we will now plot the T2 variable (you can use ADC, but that color is being displayed is hard to see here because of the size of the boxes, and the violin plots are hard to see).


I selected the options shown in Figure \@ref(fig:km1-settings) following:
```{r km1-settings, fig.cap =  "KMggplot2: options for the violin plot example.", fig.align= "center", out.width="80%", echo=FALSE}
knitr::include_graphics("bp-ggp-1b.png")
```

And that produced Figure \@ref(fig:km-vp1)
```{r km-vp1, fig.cap =  "KMggplot2: violin plot example.", fig.align= "center", out.width="100%", echo=TRUE}
...
We have done three things: color the boxes, overimpose the points themselves, and use violin plots. Here, violin plots are a little bit of an overkill, because there are not those many data points. But violin plots can sometimes be good alternatives to boxplots as they show the distribution of the data, a density plot (somewhat similar to a histogram); the marker (the circled cross) indicates the median.

Some other violin plots show also the 1st and 3rd IQRs, something that is easy to do by plotting both the violin and a boxplot in the same place;  this we need to do via the command line in R. In Figure \@ref(fig:km-vp-bp) all we do is add the line that says \texttt{geom\_boxplot(width = 0.1)} to the previous code in the R Commander console and execute it (OK, I also decrease the size of the points, so they do not interfere with us seeing the boxplots):

```{r km-vp-bp, fig.cap =  "KMggplot2: violin + boxplot example.", fig.align= "center", out.width="22cm", echo=TRUE}
.df <- data.frame(x = Brain_MR$Time, y = Brain_MR$T2,
                  z = Brain_MR$Time, s = Brain_MR$Area,
                  t = Brain_MR$Group)
.plot <- ggplot(data = .df, aes(x = factor(x), y = y, fill = z)) +
...
## Complex layouts
We have used two different systems for moderately complex layouts. **Always verify** you are plotting what you think you are plotting. Using two different systems with different syntaxes is one way to accomplish that. Another one is checking at least some of the figures with some of the numerical tabular summaries (means, for example) for the different combinations we saw above.
