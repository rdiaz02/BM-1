...
1. Open the "Brain_MR_data.xls" in Excel
2. Save it as .csv
...
We will now load the dataset corresponding to **case 2**. (If you only had the xls file, the first step would be to save data from the file “qPCR_data.xls” as a comma separated text file, “qPCR_data.csv”, to load it into R commander; we do not need to do this here, since we already have the csv file.) This data set contains the results of all four experiments (figure 2) arranged in three columns: experiment, treatment, EFN3A_levels. Note that data is already arranged as a data matrix, (i.e. “correct” way).
