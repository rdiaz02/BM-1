...
We can ask for the\(\rightarrow\) frequency counts by selecting ``Summaries \(\rightarrow\) Frequency'' distributions under the \emph{Statistics}
...
That is a contingency table. Above, we tested for differences in proportions between those treated with different drugs. But that there are no differences in proportions is the same thing as saying that the probability of healing (or not healing) does not depend on the drug given. In other words, that there is independence between Outcome and Drug. Or put it this way: we want to test whether the frequencies of Healed vs.\ Non-healed change across levels of Drug (i.e., if the frequencies of Healed and non healed are different between the drugs). So we now phrase our null hypothesis as saying that Outcome and Drug are independent\footnote{We are ignoring here the different statistical models that give rise to 2x2 tables, but these explain the different emphasis in one question or another. It is customary to differentiate between three research designs \citep{richardson2011, campbell2007}. In one design, both margins are fixed, such as the one in the tea tasting experiment behind Fisher's exact test; in another, only one of the margins is fixed, sometimes called ``comparative''; finally, in some designs no margin is fixed, and only the total sample is fixed or ``cross-sectional''.  The ``comparative'' is often associated with questions about differences in proportions and the ``cross-sectional'' with questions about association.}.
...
For large (meaning large numbers of rows and/or columns) patterns in residuals might not always be discernible and thus might not always help us much. Section \ref{sec:odds} mentions another approach.
...
Slightly more complicated are these two related issues:
\begin{enumerate}
...
There are no simple answers, but we will follow standard textbook recommendations here.
...
Now, Fisher's with simulation of p-values. We use a small number here, the default of 2000 (for real, I'd use a lot more)\footnote{Incidentally, the p-value you are likely to get is 0.0005, actually 0.00049975, which is \(1/(B + 1)\); this is because the pattern we observe is the most extreme of all the simulated ones, and we compute the p-value in the usual way: how many are as extreme as, or more extreme than, what we find. Since we have used 2000, the denominator is 2001. This is just a footnote, and you can skip it.}:
...
If you do not want to read the primary statistical literature or consult with a statistician (a statistician  that knows about this stuff) then the path of least resistance is probably to stick to the ``usual rules'' and use Fisher's test if your data do not fulfill the ``usual reals''.  This is not a cynical recommendation, but is based on fairly standard and recent textbooks making these kinds of recommendations. Moreover, many people deal with tables larger than 2x2. But then, maybe 10 years from now  we will be teaching something different because Campbell's suggestion will have become
the standard approach for 2x2 tables.  % Myself \ldots I might give Campbell's procedure a try, but then I am often more concerned about p-values that are smaller than they should be than



\subsubsection{Chi square with \texttt{simulate.p.value}?}

This is another apparent alternative: just use your usual \texttt{chisq.test} but with option \texttt{simulate.p.value = TRUE}. No reliance in the asymptotic approximation. There is catch, though: in R, when you use \texttt{simulate.p.value}, the simulation uses the same sampling model as in Fisher's exact test\footnote{This is copied verbatim from the help of \texttt{chisq.test}: ``In the contingency table case, simulation is done by random sampling from the set of all contingency tables with given marginals, and works only if the marginals are strictly positive. (...)  Note that this is not the usual sampling situation assumed for the chi-squared test but rather that for Fisher's exact test.''} There is nothing wrong with this, but this is just Fisher's test, so if you have concerns about Fisher's test % sampling model and its possible effects on power
...
We can use the standardized residuls with the code below (this is the same procedure as used by \citealp{agresti2018} in p.~41 and 138 of \citet{kateri2014} in p.~56).
...
\citet[in section 2.4]{kateri2014} provides a great overview of plots for categorical data; this book is available from UAM's library.
