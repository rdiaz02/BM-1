...
We used the argument \texttt{correct = FALSE} to prevent R from using a continuity correction. The reasons can be found, for example, in \citet[section 1.6.2, p.~18]{agresti2018}, \citet[note 1.5, p.~25]{agresti2012}, or \citet[section 1.2.3, p.~35]{bilder2024}. In what follows, we will always (intend to) avoid the continuity correction. By the way, that is what R commander does by default.
...
Now, go do a two-sample proportions test using the normal approximation. We can also quickly do it by entering the successes and total cases (i.e., healed + non haled), in other words, the info in the table, when calling the function \texttt{prop.test}:
...
\section{Goodness of fit: Comparing observed and expected frequencies}
...
\section{Contingency tables: testing independence between rows and columns}

\subsection{2x2 contingency tables}

We said above \FIXME{do}
...
So we have very strong evidence of deviations from the null in both the diet and the Hx data. We can look at the residuals, the deviations from the expected, to understand what is happening (R commander makes this available under ``Components of chi-square'').
...
For large (meaning large numbers of rows and/or columns) patterns in residuals might not always be apparent and thus might not always help us much.
...
\section{Visualizing multivariate categorical data}
...
\citet{friendly2016} is a book-length treatment of visualization of categorical data using R.
