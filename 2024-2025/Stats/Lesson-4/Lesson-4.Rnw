...
The answer was often ``use Fisher's exact test''. That, however, might not be the best idea.

\subsubsection{Fisher's exact test}
\label{sec:fishers-exact-test}

It can be used for 2x2 tables and for larger rxc tables.  It computes the exact probability of getting the observed statistics (and any other more extreme) by exhaustively examining all the possible distributions of counts in the table. The test conditions on both the row and column marginals, and for a 2x2 table this yields a hypergeometric distribution, for example for the counts in the first row and column. The test can be extended to rxc tables.

The test can become computationally unfeasible for very large tables (large in terms of dimensions, numbers of rows and columns, and/or sample size). In this case, we can obtain p-values by simulation (in R use argument \texttt{simulate.p.values = TRUE} in larger than 2x2 tables). Instead of examining exhaustively all possibilities, we repeatedly simulate outcomes (samples) under the null hypothesis (null hypothesis and, for Fisher's, fixed row and column margins) using a Monte Carlo procedure; for each such simulation we compute the statistic and then count how often the simulated statistics are larger than the observed one.
...
More sophisticated simulation schemes are possible, but that is beyond the scope of this class (and might run into other problems, like generating marginals that are 0, etc).
