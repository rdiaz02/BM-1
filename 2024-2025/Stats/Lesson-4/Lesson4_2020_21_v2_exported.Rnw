...
\paragraph{In this chapter we will learn about:}
\begin{itemize}
\item differentiate goodness of fit and analysis of independence
\item contingency tables
\item statistical test for categorical variables (Chi square test and Fisher's
exact test)
\item graphical visualization of categorical data
\end{itemize}

\paragraph{We will also learn how to generate contingency tables and perform
relevant test in R commander.}
\begin{quote}
...
\paragraph{Study the following two cases. In which way are they different to
the other cases we have analyzed so far?}
\begin{casenv}
\item To explore the physiological functions of endothelin-2 (EDN2), a research
group generated gene-targeted mouse models. The offspring of the matting
between Et2+/- animals resulted in 40 Et2 +/+ , 72 Et2 +/\textendash{}
, and 42 Et2 \textendash /\textendash{} mice\footnote{data from J Clin Invest. 2013;123(6):2643\textendash 2653.}.
Does the inactivation of this gene affect viability? In other words,
are these offspring numbers significantly different from the expected
...
\paragraph{The two cases described above have in common that they refer to categorical
...
to as frequency analysis}. Likewise, in the second case, we are measuring two nominal variables
...
\paragraph{Let's focus on the first case which is a typical (and quite boring...)
example analysis of categorical data. The question was: does the inactivation
of this gene affect viability? well, if the gene has no effect on
...
we would expect some variation due to chance. The question is, how
far from the expected frequencies are we willing to accept before
we suspect some effect of the gene knockout on viability? To answer
this question we could compare observed and expected frequencies and,
when the difference exceeds a threshold, conclude that the observed
frequencies depart from expected. This is exactly what the Pearson's\protect\footnote{there are other chi-squared test for categorical data, but this is
...
\paragraph{So, as usual, we start with the hypothesis that there is no effect
of gene deletion on viability and thus the frequencies of genotypes
are Mendelian so that any departure from the expected is due to chance
(this is our null hypothesis). Next we want to test whether the data
supports this hypothesis or disproves it in favor of the alternate
hypothesis (i. e. there is effect on viability). To this end, we have
to calculate the difference between the observed and expected values
to get a measure (an statistic) of how far our data departs from the
expected. This statistic is the chi-square($\chi^{2}$), that is calculated
...
whether the frequencies were higher or lower than expected. Pearson's
...
\paragraph{Once we have an statistic that reflects the agreement between the
...
\cprotect\paragraph{Note however that shape of the chi-square($\chi^{2}$) distribution
...
\paragraph{Getting back to our problem (case \#1), to determine if the offspring
...
\paragraph{We can ask for the frequency counts by selecting \emph{Summaries
...
\paragraph{Within this same menu we can perform a chi-square goodness of fit
...
\paragraph{Note that we did not have to explicitly indicate the degrees of freedom
...
the same as concluding that ratios are Mendelian. In figure, you'll
find the R code to perform this test. }
...
\paragraph{Now, let focus on the second case above in which we wanted to determine
...
\paragraph{To analyze the interaction between categorical variables, we use
...
\paragraph{Although we will only deal with two variables in this class, you
...
\paragraph{Read the data in file \textquotedblleft Hyp\_effect.csv\textquotedblright{}
...
\paragraph{Because the resulting probability is small, you reject the hypothesis
that presence of HRE and gene expression are independent. }

\paragraph{Note that, to calculate the degrees of freedom, for contingency tables
...
\paragraph{Mosaic plots are a very informative two-dimensional representation
...
\citet{Xu2010} (by the way, this reference is a nice review of categorical
...
\paragraph{It is important to note that we can apply the chi-square test provided:}
\begin{itemize}
...
\item Expected cell count \textendash{} Adequate expected cell counts. No
more than 20\% of the expected frequencies are less than five\footnote{A common rule is 5 or more in all cells of a 2-by-2 table, and 5 or
...
small sample size avoids getting an accurate estimation of the frequencies.}.
\end{itemize}

\paragraph{There are several alternatives (other than increasing sample size!),
when the fourth (and/or third?) condition is not met:}
\begin{enumerate}
...
\item Randomization test. This test is unique in that it does not calculates
...
more than 20\% of the cells, the randomization test should be used
instead. %
\begin{comment}
...
null hypothesis.
\end{comment}
\item Fisher's Exact test. It can be used for 2x2 contingency tables regardless
the sample size. It does not calculates any statistic but instead
calculates the probability of the observed distribution directly.
However, unlike the randomization tests, it does not relies on random
sampling, instead it computes the exact probability of getting the
observed distribution (and any other more extreme) by ``counting''
all the possible combinations that lead to the observed frequencies.
\begin{comment}
...
\section{Randomization test and Fisher's exact test in R}
...
\paragraph{Unfortunately, the randomization test can not be accessed from R
...
\ref{fig:Heterogeneity,-randomization-and}).}

\begin{figure}[h]
\caption{\protect\label{fig:Heterogeneity,-randomization-and}Heterogeneity,
randomization and Fisher's test in R }

<<randomization test,echo=-1,fig.width=4,fig.height=4>>=
...
\paragraph{The statistical test we have seen indicate when there is a statistically
...
\paragraph{Lets explore the strength of associations in further detail. Imagine
...
\paragraph{As we have seen above, R can compute the contingency table from raw
...
\paragraph{To enter these values in a tabular form go to Statistics->Contingency
...
\paragraph{Please take a look at the OR in the output of the Fisher's exact
...
\section{\protect\label{sec:Other-test-for}Other test for categorical data}


\paragraph{Finally, be aware that we have only scratched the surface of categorical
...
\section{Exercises}
\begin{xca}
A common assumption in DNA sequence analysis is that the frequency
...
sep
\begin{xca}
You suspect that the presence of a disease is associated to a certain
...
sep
\begin{xca}
You want to know if there is a bias in the gene content for the +
...
% \bibliography{/home/luis/Documents/Lab_DB/Bibliography/library.bib/library}
