...
The two cases described above have in common that they refer to categorical
...
Let's focus on the first case which is a typical (and quite boring...)
...
So, as usual, we start with the hypothesis that there is no effect
...
Unfortunately, the randomization test can not be accessed from R
