...
%% THIS FILE IS NO LONGER USED. Left here for historical reasons only!!
...
\paragraph{Study the following two cases. In which way are they different to the other cases we have analyzed so far?}
\begin{casenv}
...
We will now use the data in \citealp[Exercise 8.3, p.~154 of][]{Dalgaard2008}.
...
Exercise3 <- data.frame(drug = c("Piren", "Trithi"),
                        outcome = c("Healed", "NotHealed"))
@
