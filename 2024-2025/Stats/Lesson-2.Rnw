...
When we have paired data, the idea here is to assess whether the within-pair differences are symmetrically distributed around zero. As we are talking about symmetry, this means that this test can be used \textbf{only with interval scale} data (see also section \ref{wpi}).
