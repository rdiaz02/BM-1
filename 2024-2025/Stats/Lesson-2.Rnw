...
If you have access to it, Rupert Miller ``Beyond Anova'' (Chapman and Hall, 1997), bottom of p.~8 and p.~9 discusses this issue. In class, we will quickly discuss the code below:
...
And this emphasizes something else: when examining if assumptions are reasonable, think about what you are examining. Here, the distribution that matters is the distribution of the within-individual differences, not each of the separate distributions \texttt{d1} or \texttt{d2}. Your test is about individual differences, not about \texttt{d1} or \texttt{d2}.
