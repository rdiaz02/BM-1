...
(This section is probably too long for this course. We will cover it quickly, and
...
interactions, results of these procedures are often similar, though not
necessarily identical (with unbalanced designs, specially heavily unbalanced,
\Rfunction{glht} is often preferable --- see the discussion in Everitt and
Hothorn, chapter 5 and 14).
...
is, as we anticipated, a complicated issue.


Another package that might be worth checking is \CRANpkg{emmeans}
...
\Burl{https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html}). 
...
\subsection{Does order always matter? Further examples and details}
