...
are often (but not always!) variables we do not want to adjust for. Likewise for
...
Trying to understand  the above patterns:
...
<<f2d12ww,echo=FALSE, fig.pos = 'H',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='Trying to make sense of those patterns, 1b'>>=
library(lattice)
xyplot(tumor.size ~ exposure | drug, data = tgd2)
@ 
...
<<f2d12z,echo=FALSE, fig.pos = 'H',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='More non-constant variance. A regression model.'>>=
suppressWarnings(try(rm(list = ls()))) 
...
<<f2d77z,echo=FALSE, fig.pos = 'H',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='And more non-constant variance of residuals plus other issues'>>=
suppressWarnings(try(rm(list = ls()))) 
suppressWarnings(try(rm(X, U, Z, x1, y1)))
...
sex <- factor(rep(c("Male", "Female"), c(60, 60)))
smoking <- factor(rep(rep(c("No", "Yes"), c(30, 30)), 2))
hdl <- rep(c(3, 2, 6, 4), rep(30, 4))
hdl <- hdl + rnorm(length(hdl), sd = 2)
hdl[c(1, 21, 13)] <- hdl[c(1, 21, 13)] + 27
hdl1B <- data.frame(hdl, sex, smoking)[1:110,]
rm(hdl, sex, smoking)
...
% %% zz: fixme: label plots clearly, using mar as above,
% %% and add legends to plots
% %% and why isn't myAdditive2b not showing?
