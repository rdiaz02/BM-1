...
\subsection{Diagnostics: more examples with regression models}\label{diag-more-reg}

<<f2d1z,echo=FALSE, fig.pos = 'H',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='Non-constant variance'>>=
...
<<f2d12z,echo=FALSE, fig.pos = 'H',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='More non-constant variance'>>=
suppressWarnings(try(rm(list = ls()))) 
suppressWarnings(try(rm(X, U, Z, x1, y1)))

set.seed(3)
x1 <- runif(200, 1, 100)
y1 <- 4 + 3 * x1 + rnorm(200, sd = 0.1 * (x1^0.7))
...
<<f2d12z,echo=FALSE, fig.pos = 'H',fig.width=5.8, fig.height=5.8, fig.cap='', fig.lp="fig:", fig.cap='Weird things happening'>>=
suppressWarnings(try(rm(list = ls()))) 
suppressWarnings(try(rm(X, U, Z, x1, y1)))

set.seed(39)
x1 <- runif(200, 1, 100)
y1 <-  x1 + rnorm(200, sd = 0.1 * (1/(x1^1.7)))
x1y1 <- data.frame(x1, y1)
x1y1lm <- lm(y1 ~ x1, data = x1y1)
par(mfrow = c(2, 2), oma=c(0,0,3,0))
plot(x1y1lm)
