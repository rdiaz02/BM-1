...
\begin{itemize}
\item The threshold we use for saying ``the means differ'' ($\alpha$ level, or Type I error).
...
might be testing a slightly different null hypothesis. Whether or not to use can
sometimes be a contentious issue. Some considerations that come into play are:
