...
The purpose of these notes is to try to clarify  questions about ``what
...
personal action (should I start drinking a little bit of wine?).  Intuitively, if
a variable, X, has a causal effect on a variable, Y, manipulating X will change
the value of Y.

And what do we mean by causal effects? ``(...) estimation of causal effects in
populations, that is, numerical quantities that measure changes in the
distribution of an outcome under different interventions.'' \citep[p.\
v]{hernan2020}.
...
examples of this pattern below: \ref{dag-structs}). A similar well known
phenomenon is the very real association between ice cream sales and homicide
rates (increasing ice cream sales positively correlated to homicide rates, at
least in some places of the world).
...
\qref{modified-basic} very, very quickly, except for structure f). The main
objective of this document is to allow you to understand why we should or
shouldn't adjust for covariates in the examples we give (\qref{chol-exercise},
\qref{fungus-ex}, \qref{fungus-collider}, \qref{pretreatment-no},
\qref{cause-of-cause}, \qref{amplification}, \qref{bwparadox}). Section
\qref{dag-structs}, \qref{backdoor-basics} are there just to explain why.

More complex examples are provided in \qref{direc-indirect}, but you can skip
this if you want.
...
<<plot_dag_1, fig.width = 4, fig.height=2, fig.lp='fig:', results = 'hide', echo=FALSE, fig.align = 'center', fig.cap='Cholesterol, Exercise, Age example',out.width='10cm'>>=
...
let us add some descendants and ancestors to the previous structures them and see
what happens. If you want, you can read this section very quickly and skip over
all the ``in addition''. The \textbf{most important to pay attention to is
...
what all the examples here show :-) ).
...
to correctly interpret the effects from your experimental manipulations. And
...
\citet{pearl2016} presents a very interesting
