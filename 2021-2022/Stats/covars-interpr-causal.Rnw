...
The interpretation we often want to give is a causal one. For example, in a
...
rates (increasing ice cream sales is positively correlated with homicide rates, at
...
not) through these pipes (see \citealp[][ch.~6]{hernan2020}; in Miguel Hern\'an's
edX
...
\item X and Y are independent \textit{if we adjust for} Z (or \textit{adjusting
    for} Z).\footnote{X and Y are independent \textit{if we hold Z constant} can be
