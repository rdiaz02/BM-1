...
We will use $U$ for unmeasured variables
...
to an increase in Cholesterol. How would we analyze this data?
...
<<plot_dags_1_2, fig.width = 10, fig.height=5, fig.lp='fig:', results = 'hide', echo=FALSE, fig.cap='Introductory figures'>>=
...
N <- 1e4 
################## Common_cause
common_cause <- data.frame(Age = runif(N, 10, 50))
...
\clearpage
Let's analyze the data with and without adjusting for Age or Food
consumption. First for case a)
...
In case a) we must adjust for Age. In case b), if we want to measure the total
...
analysis of what has been called Lord's paradox\footnote{This same paradox has
  been presented in many different places by other authors, and also by Pearl in
  other work; but the presentation one in \citealp{pearl2016} I find much, much,
  clearer and insightful than the one in p.\ 85 of \citealp{pearl_causal_2016} or
  pp.\ 212-215 ---chapter 6--- of \citealp{pearl2018}}.

As Pearl explains, this paper ``address(es) the general methodological issue of
whether adjustments for preexisting conditions is justified in group comparison
applications''. A nice feature of this paper is that he presents both the
...
whereas in the second we have confounding problem. Even if the structure of the
different presentations can seem misleadingly similar, they are fundamentally
different. In mediation case, whether or not to adjust for covariates depends on
whether we want the direct effect (this is what we would get if we adjust for the
covariate) or the total effect (what we get when we don't). In the confounding
case, of course, we must adjust for the covariate. 
