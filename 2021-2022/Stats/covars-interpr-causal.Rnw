...
\subsection{The backdoor criterion: basics}\label{backdoor-basics}
...
complete procedure in full detail, so that you have it there. And it is very well explained in:
\begin{itemize}
...
what are called backdoor paths. 
...
<<plot_dag_ptr, fig.width = 4, fig.height=4, fig.lp='fig:', results = 'hide', echo=FALSE, fig.cap='A pretreatment collider',out.width='9cm', fig.pos= "!h">>=
...
<<plot_dag_zxy, fig.width = 4, fig.height=2, fig.lp='fig:', results = 'hide', echo=FALSE, fig.cap='Cause of a cause',out.width='8cm', fig.pos= "!h">>=
coordinates(ccc) <- list(x = c(Z = 1, X = 2, Y = 3),
                         y = c(Z = 0, X = 0, Y = 0))

drawdag(ccc, xlim = c(0.5, 3.5), ylim = c(0, 1.3))
## text(x = 2, y = 1.2, labels ="a)", cex = 1.2)
@ 


Short answer: no. If you do, it will increase the noise of the estimate of the
effect of X on Y. (Code to see how adjusting for Z leads to increased standard
errors is provided in file \texttt{Z\_X\_Y\_adjust\_Z.R}, in the repo.)

Longer answer: no, but sometimes people adjust for Z because Z might be a common
cause of X and Y. Here the argument is ``if I do not adjust for Z and it is a
...
Now consider this DAG (U is an unmeasured variable): Figure \ref{fig:plot_dag_bias_amp}
...
<<plot_dag_bias_amp, fig.width = 4, fig.height=2, fig.lp='fig:', results = 'hide', echo=FALSE, fig.cap='Cause of a cause and bias amplification',out.width='8cm', fig.pos= "!h">>=
coordinates(bamp) <- list(x = c(Z = 1, X = 2, Y = 3, U = 1),
                         y = c(Z = 0, X = 0, Y = 0, U = -.5))

drawdag(bamp, xlim = c(0.5, 3.5), ylim = c(0, 1.3))
## text(x = 2, y = 1.2, labels ="a)", cex = 1.2)
@
...
phenomenon is also mentioned in \citet{vanderweele2011}. (And in case you care: Z
could be used as an instrument or instrumental variable, but when adjusting for
it, it is not being used as an instrument.)
...
I copy from \citet{hernandez-diaz2006}:
...
effect. The following are three possible causal DAGs that would lead to this
paradox.
...
weight can be caused by conditions other than smoking. Once we know a baby has
low birth weight, if we know it is the child of a smoking mom, then it is less
likely that the baby has those other conditions (this is the usual ``explaining
away'' of colliders). In fact, we do not need LBW to have any effect on
...
show lower death rates. But recommending smoking to pregnant women would be a
very bad idea.
...
As it says: what follows gives additional details on some issues above. I follow
no particular order.
...
Look at the following DAG (this is from \citealp[section 3.7, pp.\ 75 and
ff.\]{pearl_causal_2016}; a similar example is shown in \citealp[section 6.3.2,
p.\ 180--182]{rethinking_2020}\footnote{Though in \citealp{rethinking_2020} the
  explanation of the problem I find possibly misleading, since the difference
  between the direct and indirect effects is not emphasized enough}) in Figure
\ref{fig:dag_direct}:


<<dag_direct, results='hide', echo=FALSE,fig.width = 4, fig.height=4, fig.lp='fig:', fig.cap='Direct effects',out.width='9cm', fig.pos= "!h">>=
op <- par(mfrow = c(1, 2))
dire <- dagitty("dag {
...
drawdag(dire, xlim = c(0.5, 3.5), ylim = c(-.9, 1.3))
...
<<dag_direct_conf, results='hide', echo=FALSE,fig.width = 4, fig.height=4, fig.lp='fig:', fig.cap='Direct effects and confounding',out.width='9cm', fig.pos= "!h">>=
op <- par(mfrow = c(1, 2))
dire3 <- dagitty("dag {
Gender -> Qualification
Gender -> Hiring
...
drawdag(dire3, xlim = c(0.5, 3.5), ylim = c(-.9, 1.8))
@ 
...
If you wanted an unbiased estimate of the direct effect, you would need to
measure and condition on both Qualification and SocioecStatus.
...
mediation. A book-length treatment in \citet{vanderweele2015}.
...
direct effect and how not adjusting for it gives the total effect. This is for a
...
precisely because treatment assignment is random. So go an redraw the very first
