...
These just extends what we saw in the previous section: instead of a 2x2 table, we have a two-dimensional table but rows and/or columns can have more than two categories or types. So we still have \(\chi^{2} = \sum \frac{(n_{ij} - \hat{\mu}_{ij})^2}{\hat{\mu}_{ij}}\).
...
The main concerns with the use of Fisher's exact test in the literature  (when maybe you chi-square approximation is not that bad after all) are:
\begin{itemize}
\item Conservatism.
