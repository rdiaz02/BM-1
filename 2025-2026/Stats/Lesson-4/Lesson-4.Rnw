...
We are given a p-value and a confidence interval. This relies on a normal approximation to the binomial. This approximation is justified if the expected number of successes (in our example, cases with asthma) and failures are both larger than 10. This is the case for us (\(0.15\ *\ 215 = 32.25 \) and \(0.85\ *\ 215 = 182.75\)). Note, though, that this interval is often not symmetrical, and this (not being symmetrical) is a good thing\footnote{See \citet[p.~18 and section 1.3.5]{agresti2018}. You can verify that it is not symmetrical using, for example, \texttt{x = 6, n = 7} in the arguments to \texttt{prop.test}. But more generally, there are several different ways to obtain CIs for binomial proportions and using the binomial distribution itself, as we do in section \ref{sec:exact-binomial-test}, does not guarantee ``the best'' CI either;  in fact, finding two-sided CIs is not a well defined problem. Choosing the right procedure might be important for small sample sizes or estimates very close to 0 or 1. See \citet{agresti2018}, \citet[sections 1.1.2 and 1.1.3]{bilder2024}, exercise 8.5 in \citet{Dalgaard2008}, and R packages \texttt{binom}, \texttt{exactci} and \texttt{PropCIs}.}.
...
ratios do not fit the expected values)\protect\footnote{Residuals are the difference between the observed and expected values.
  ...
  residuals are calculated as $\frac{observed-expected}{\sqrt{expected}}$. These are the residuals, \textbf{not} the standardized residuals we will see in section \ref{sec:residuals}.}.
...
The ``comparative'' design is often associated with questions about differences in proportions (differences in the binomial or multinomial distributions) and the ``cross-sectional'' with questions about association. These differences sometimes affect how we analyze the data, but for the contingency tables we discuss here they will have no effect.
