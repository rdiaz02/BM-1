...
library(Rcmdr)
library(formatR) ##  required by the chunk option tidy = TRUE
@
...
This work is Copyright, \copyright, 2020, Luis del Peso Ovalle, \copyright, 2024, 2025, Ramon Diaz-Uriarte, and is licensed under a \textbf{Creative Commons } Attribution-ShareAlike 4.0 International License: \Burl{http://creativecommons.org/licenses/by-sa/4.0/}.
...
Study the following  cases. In what way are they different to the other cases we have analyzed so far?
...
\item Gene expression profiling experiments demonstrate that exposure of
  ...
  regions. Does HIF/HRE show any preference for up- or downregulation? (See section \ref{categ_data} for a higher-level description of this experiment.)

  %% RDU: I think this is the thing: you do an experiment and subject cells to hypoxia. You divide the genes between those that are upregulated, downregulated or non-regulated. In each group, you examine if they are HRE+ or HRE-, whatever that means (if a gene has HRE). Yes, see next: "In the gene expression example ..."
...
\section{Categorical data}\label{categ_data}
...
We will use the data set described in \citet[section 8.1 of][]{Dalgaard2008}: 39 of 215 randomly chosen patients are observed to have asthma. We want to test the null hypothesis that the probability of asthma is 0.15 (why? maybe because 0.15 is the reference level for that region).
...
\subsection{Normal approximation (score test)}
...
@
%% From Agresti, p. 18: "The X-squared value in the R output is the score test statistic, having a chi-squared null distribution with df = 1."



We are given a p-value and a confidence interval\footnote{This is using the score statistic; when comparing two proportions, \texttt{prop.test} uses a Wald confidence interval; see, e.g., \citet[p.~25, section 2.5]{kateri2014} or \citet[pp.~18 and 31]{agresti2018}}. This relies on a normal approximation to the binomial. This approximation is justified if the expected number of successes (in our example, cases with asthma) and failures are both larger than 5. This is the case for us (\(0.15\ *\ 215 = 32.25 \) and \(0.85\ *\ 215 = 182.75\)). Note, though, that this interval is often not symmetrical, and this (not being symmetrical) is a good thing\footnote{See \citet[p.~18 and section 1.3.5]{agresti2018}. You can verify that it is not symmetrical using, for example, \texttt{x = 6, n = 7} in the arguments to \texttt{prop.test}. But more generally, there are several different ways to obtain CIs for binomial proportions, and using the binomial distribution itself, as we do in section \ref{sec:exact-binomial-test}, does not guarantee ``the best'' CI either;  in fact, finding two-sided CIs is not a well defined problem. Choosing the right procedure might be important for small sample sizes or estimates very close to 0 or 1. See \citet[sections 1.3.4 and 1.3.5]{agresti2018}, \citet[sections 1.1.2 and 1.1.3]{bilder2024}, exercise 8.5 in \citet{Dalgaard2008}, and R packages \texttt{binom}, \texttt{exactci} and \texttt{PropCIs}.}.


We used the argument \texttt{correct = FALSE} to prevent R from using a continuity correction. The reasons can be found, for example, in \citet[section 1.6.2, p.~18]{agresti2018}, \citet[note 1.5, p.~25]{agresti2012}, \citet[section 2.5]{kateri2014} or \citet[section 1.2.3, p.~35]{bilder2024}; basically, it is a very conservative procedure. In what follows, we will always  avoid the continuity correction. By the way, that is what R Commander does by default.
...
%% The CI of the binomial is done by summing two tail areas of 0.025, from the binomial.
%% The CI of the prop.test is done by finding the critical regions of each of the 0.25 regions, using the score statistic.
...
There are several ways to enter these data and get the confidence interval and tests output. We could create a data frame, for example this way (it is OK if you do not understand how I am creating the data ---these are just R syntax details):


<<>>=
ex3 <- expand.grid(Drug = c("Pirenzepine", "Trithiozine"),
...
Now, select ``Exercise3'' as the active data, and do a two-sample proportions test and obtain a confidence interval using the normal approximation via the menus.

<<>>=
local({
    .Table <- xtabs(~Drug+Outcome, data=Exercise3)
    cat("\nPercentage table:\n")
...
%% " We can use the prop.test() function to perform the equivalent score and Pearson chi-square tests.  ", from  Bilder and Laughin, p. 37.
...
And, before leaving this section, it is arguable that for many problems, odds ratios are more informative than differences between proportions; see section \ref{sec:odds}.

\section{Goodness of fit: Comparing observed and expected frequencies}\label{sec:goodness-fit}

Let us return to the mouse example:  To explore the physiological functions of endothelin-2 (EDN2), a research group generated gene-targeted mouse models. The offspring of the matting between Et2+/- animals resulted in 40 Et2 +/+ , 72 Et2 +/\textendash{} , and 42 Et2 \textendash /\textendash{} mice\footnote{Data from Chang et al. J Clin Invest. 2013;123(6):2643--2653. \Burl{https://www.jci.org/articles/view/66735}}.  The question was: does the inactivation of this gene affect viability?
...
The test statistic we will use is the chi-square ($\chi^{2}$) statistic, calculated
according to the formula:

\[\chi^{2}=\sum_{i}\frac{\left(observed_{i}-expected_{i}\right)^{2}}{expected_{i}}\]
where \(i\) refers to the different categories.
...
ratios do not fit the expected values)\protect\footnote{Residuals are the difference between the observed and expected values.
  ...
  to enable individual residuals to be compared relative to one another. We return to residuals in section \ref{sec:residuals}.}.
...
Now, all we need to do is assess how likely it is, under \(H_0\), that we find a value of the statistic as large as, or larger than, the value we found. We do this by using the chi-square($\chi^{2}$) distribution (see figure \ref{fig:Chi-squared-probability-distribu}). In contrast to what we did with the two-tailed t-test, we only look at the right tail of the distribution. Why do you think this is the case? (This is also what we did with the F-statistic, by the way).
...
Getting back to our problem to determine if the offspring
...
We can ask for the frequency counts by selecting ``Summaries \(\rightarrow\) Frequency'' distributions under the \emph{Statistics}
...
Contingency tables allow us to organize and visualize categorical data. These are tables with the levels of one variable in one dimension across the levels of the other variable in the second dimension. Table \ref{tab:Frequency-of-HRE} is a contingency table that summarizes the observed data. Some of the simplest contingency tables are 2x2 tables: 2 rows and 2 columns. For example, what we had in section \ref{sec:two-prop-conf} and to this we go next.


\subsection{2x2 contingency tables}\label{sec:2x2}

We used these data in  section \ref{sec:two-prop-conf}. Let us see it again

<<>>=
xtabs(~ Drug + Outcome, data = Exercise3)
@

That is a contingency table. Above, we tested for differences in proportions between those treated with different drugs. But that there are no differences in proportions is the same thing as saying that the probability of healing (or not healing) does not depend on the drug given. In other words, that there is independence between Outcome and Drug. Or put it this way: we want to test whether the frequencies of Healed vs.\ Non-healed change across levels of Drug (i.e., if the frequencies of Healed and non healed are different between the drugs). So we now phrase our null hypothesis as saying that Outcome and Drug are independent.


Under \(H_0\), i.e., if there is independence between Outcome and Drug, then, for example, the probability of both being healded and having taken Pirenzepine (shortened to ``Piren'' in the next expressions) is the product of each. And therefore the expected number of people who took Piren and are Healed should be the product of those two probabilities times the total sample size. In other words, under \(H_0\), the expected number of people who are Healed and took ``Piren'' should be \( P(Piren)\ P(Healed)\ n\), where \(n\) is the total sample size (in our case 61). We can write it as: the expected number of people in the ``Healed and Piren'' cell should be \(\hat{\mu}_{Healed,Piren} =    \hat{\pi}_{Piren} \ \hat{\pi}_{Healed}\ n\).

We wrote the above for one of the cells, but for any table, where we index rows with \(i\) and columns with \(j\), we can write this more generally as \(\hat{\mu}_{ij} = n\ \hat{\pi}_{i+}\ \hat{\pi}_{+j}\).


Now, \(\hat{\pi}_{Piren}\), or \(\hat{\pi}_{i+}\), for \(i = Piren\), is the marginal probability of Piren: the sum of the entries in the row that says Piren divided by the total. So \(\frac{23 + 7}{61}\). And similarly for \(\hat{\pi}_{Healed}\), or \(\hat{\pi}_{+j}\).

Therefore, in our 2x2 table we can compute the four \(\hat{\mu}_{ij}\), the four expected numbers in each cell under \(H_0\).

And we are done. We now compute the \(\chi^{2}\) statistic as \(\sum \frac{(n_{ij} - \hat{\mu}_{ij})^2}{\hat{\mu}_{ij}}\).
...
By the way, notice the similarities between what we have done and examining interactions in a two-way ANOVA. And even if we can frame this problem, with these data, as one of comparing proportions, we can use 2x2 tables where the idea of ``comparing proportions'' is not the most natural. With contingeny tables, 2x2 or larger, we are examining for independence of rows and columns. Let's move on to larger tables then.
...
This just extends what we saw in the previous section: instead of a 2x2 table, we have a two-dimensional table but rows and/or columns can have more than two categories or types. So we still have \(\chi^{2} = \sum \frac{(n_{ij} - \hat{\mu}_{ij})^2}{\hat{\mu}_{ij}}\).
...
of \(I\) are the same regardless the levels of \(J\). Again, this is equivalent to testing independence of rows and columns.
...
Hx$Effect <- factor(Hx$Effect,
                    levels = c("Upregulated", "Non_regulated",
                               "Downregulated"))
...
You should be able to interpret the output from above. We did not talk about degrees of freedom explicitly. They are \(df = (I - 1) (J - 1)\), where \(I\) is the number of categories in the first variable, and \(J\) in the second. So in this example \(2 = (2 - 1)\ (3 - 1)\).
...
The same logic applies to tables of more than two dimensions, say Drug x  Outcome x Exercise.  Now, we can have dependencies between pairs of variables, or among the three. For example, each drug might have a different effect on outcome, and exercise might affect this difference. We will not pursue this here. Arguably, though, except for very specific problems (the Mantel-Haenzsel test mentioned below being one of them), it is probably a better idea to directly approach this using models (logistic, logit, and loglinear, for binomial, multinomial, and Poisson response variables, respectively); this is also  mentioned in section \ref{sec:glms}.



\subsection{Sampling models: details}
\label{sec:sampl-models:-deta}

I originally had this as a footnote. But I think it is important enough that it deserves to be here. In our discussion above we are ignoring the different statistical models that give rise to contingency tables, but these explain the different emphasis in one question or another. It is customary to differentiate between three research designs (for example, \citealp{richardson2011, campbell2007} and common textbooks about categorical data analysis, including \citealp[section 2.1.4]{agresti2018}, \citealp[section 2.1]{agresti2012} ---a very detailed explanation that includes discussion of sampling models, observational vs.\ experimental studies, and case-control, retrospective, prospective, and cohort studies---, \citealp[section 1.2]{kateri2014}, \citealp[section 2.2]{tang2023a} ).

In one design, both margins are fixed, such as the one in the tea tasting experiment (or lady tasting tea) behind Fisher's exact test (and it is the design that naturally leads to the hypergeometric distribution). We discuss this in section \ref{sec:fishers-exact-test}\footnote{This comes from the Wikipedia, \Burl{https://en.wikipedia.org/wiki/Lady_tasting_tea}:
``The experiment provides a subject with eight randomly ordered cups of tea – four prepared by pouring milk and then tea, four by pouring tea and then milk. The subject attempts to select the four cups prepared by one method or the other, and may compare cups directly against each other as desired. The method employed in the experiment is fully disclosed to the subject. '' We now cross-classify each of the 8 cups as ``tea was truly served first (yes or no)'' with ``lady said tea was truly served first (yes or no)''. Since the lady knows about the design, she will say four had tea poured first: so the margins are fixed, in this case each row has a margin of 4 and each column of 4.}.



In another design, only one of the margins is fixed, sometimes called ``comparative''. For example, we treat 30 patients with Pirenzepine and 31 with Trithiozine, as in section \ref{sec:2x2}. For each row of the table we have a binominal distribution. But we would have had a multinomial distribution for each row if the response had had more than two categories, instead of the two of ``Healed'' and ``NotHealed''.


In a third design, no margin is fixed, and only the total sample is fixed; this is sometimes called the ``cross-sectional'' design. For example, we stop at the door of the cafeteria and decide to look at the first 60 students; we cross-classify each according to two categories, Gender (woman, man, non-binary) and Coffee (regular, decaff, no), which gives us a 3x3 table. Arguably, the gene expression/HRE study is of this kind (with a 3x2 table). Of course, this type of sampling can also lead to 2x2 tables: we go back to the cafeteria and now we cross-classify the first 60 customers according to two binary categories: Student (yes or not) and Payment (credit card or cash).

% In some studies, the sample size is not fixed at all. For example, we could be interested in the mutagenic effects of certain drugs, and we count the number of mutations in a stretch of DNA under the different combinations of those drugs. Or we are interested in the number of bacteria cells in foods with different treatments. These lead naturally to a Poisson model.

The ``comparative'' design is often associated with questions about differences in proportions (differences in the binomial or multinomial distributions) and the ``cross-sectional'' with questions about association. The differences between the ``comparative'' and ``cross-sectional'' designs sometimes affect how we analyze the data, but for the contingency tables we discuss here they will have no effect. (The ``both margins fixed'', above, we will discuss later again).
...
So we have very strong evidence of deviations from the null in the Hx data and some evidence in the diet data. This might tell us there are deviations from independence, but we would like to examine what is behind these deviations. What cells are deviating from the expected or how strong is the association. Moreover, with large sample sizes it
...
We can obtain the Pearson residuals using \texttt{residuals}; the sum of their squared values is equal to the value of the $\chi^{2}$ statistic. But standardized residuals (or standardized Pearson residuals), that we can obtain using \texttt{stdres}, are often much more useful. The different residuals, and why to prefer the standardized Pearson residuals is very clearly explained in  \citet[section 2.2.4]{kateri2014} (also discussed in \citealp[p.~39]{agresti2018}, \citealp[p.~81]{agresti2012}, and \citealp[p.~183]{bilder2024}). Standardized residuals are asymptotically standard normal under \(H_0\)\footnote{And this holds for both multinomial and independent Poisson sampling: \citet[section 2.2.4]{kateri2014}}; thus, a cell with a standardized residual with an absolute value larger than around 2 (when few cells in the table) or 3 (when the table has many cells) indicates lack of fit of \(H_0\) in the given cell\footnote{Why do we require larger deviations for more cells? To prevent from finding one large residual just by chance.}.
...
<<res_hx>>=
## Squared residuals, as obtained from R Commander

chisq.test(Hx_tab, correct = FALSE)$residuals^2
## Note that their sum is the chi-square statistic
sum(chisq.test(Hx_tab, correct = FALSE)$residuals^2)
chisq.test(Hx_tab, correct = FALSE)
...
For large (meaning large numbers of rows and/or columns) patterns in residuals might not always be discernible and thus might not always help us much. Section \ref{sec:odds} briefly mentions another approach.



\section{Strength of association: odds ratios and relative risks}\label{sec:odds}
...
In 2x2 tables, the relative risk or the risk ratio is \(\pi_1 / \pi_2\), where \(\hat{\pi}_1 = \frac{a}{a + b}\), \(\hat{\pi}_2 = \frac{c}{c + d}\). So we are comparing how much larger the probability of recovery is for the patients in the Drug group relative to those in the control group. This is a sensible thing to do. Can you think why?
...
Another very common and statistic, widely used, for example, in epidemiology, is the odds ratio. The odds is defined as \(\frac{\pi}{1 - \pi}\). The odds ratio is then \(\frac{\text{odds}_1}{\text{odds}_2}\). In the table above, the estimate of the odds ratio is thus: \(\hat{\theta} = \frac{a/b}{c/d} = \frac{a\ d}{b\ c}\)
...
Odds ratios and relative risks are discussed in \citet[sections 2.1.5 adnd 2.2.5]{kateri2014}, \citet[pp.~39 and ff]{bilder2024} and \citet[pp.~29 and ff]{agresti2018} (and it is possible to obtain small sample confidence intervals for them; e.g., \citealp[p.~50]{agresti2018}).
...
\section{Assumptions of the chi-square test, minimal expected counts, Fisher's exact test et al.}
...
This should not come as a surprise: we assume all \emph{observations} are independent\footnote{Do not mistake it for independence of \emph{variables.}}. For example the genotype of a mouse does not affect the genotype of other mice or the presence/absence of HRE in a given gene has no effect on other genes. Similarly we do not have the same individual measured multiple times, or sets of related/paired individuals, etc. In case they are not independent, other procedures should be used (for example, the McNemar's test for two related groups; see \ref{sec:Other-test-for}).
 ...
 More controversial are these two related issues:
\begin{enumerate}
...
A common rule is that no more than 20\% of the expected (\textbf{expected, not observed!}) frequencies should be less than five; another is 5 or more in all cells of a 2-by-2 table, and 5 or  more in 80\% of cells in larger tables, but no cells with zero expected  count. In \citep[section 2.4.7, p.~42][]{agresti2018} says ``To be cautious, you should instead use a small-sample test whenever at least one expected frequency is less than 5.''

% Some authors argue that these rules might be overly restrictive and the details can depend strongly on the size of the table (numbers of rows and columns) and the total sample size.

And \textbf{what can we do if expected counts are too small?} The standard answer is ``use Fisher's exact test''. % That, however, might not be the best idea according to some.

\subsection{Fisher's exact test}
\label{sec:fishers-exact-test}

Fisher's exact test can be used for 2x2 tables and for larger rxc tables. Conditioning on the observed row and column margins, we compute the two-tailed p-value as the sum of the probabilities of all the tables that have a probability as extreme as, or more extreme than, the one of the table we observed. The probability of each table is obtained, in the 2x2 case, directly from a hypergeometric distribution\footnote{For one-tailed tests on 2x2 tables we can just focus on the number of cases in the first cell, i.e., the number of observations in the cell in the first row and column, \(n_{11}\). For fixed row and column margins, given \(n_{11}\) the rest of the entries are known. Then, we can just count how many tables there are with \(n_{11}\) as extreme or more extreme than the one we observed.}. For larger tables (e.g., 3x4) similar approaches exist, using the multivariate hypergeometric distribution\footnote{For details, see, for example, \citet[section 16.5.2]{agresti2012}, \citet[section 2.4.1.4]{tang2023a}, \citet[section 6.2.1, p.~4.4.6]{bilder2024}.} and they are implemented in R in function \texttt{fisher.test}.


To take the mystery out of this procedure, this is the classical tea testing example. I copy a couple of tables from \citet{agresti2018} (pp.~47 and 48):
...
In each row, the probability is coming from a hypergeometric distribution. This is just a detail (i.e., not in the exam) to show these numbers (look at the help for \texttt{dhyper})
<<hgex1>>=
n11 <- 0:4
sapply(n11, dhyper, m = 4, n = 4, k = 4)
@


A one-sided P-value, had we observed 3 cases in \(n_{11}\) would be \(0.229 + 0.014\) (i.e., the entries with \(n_{11}\) equal to 3 and 4, as those are the ones as extreme as the observe, because they have a probability \(\le 0.229\)). A two-tailed test would be \(0.014 + 0.229 + 0.229 + 0.014\) (all entries with probabilities \(\le 0.229\) in both directions, so \(n_{11}\) with value 0, 1, 3, 4).
...
The test can become computationally unfeasible for very large tables (large in terms of dimensions, numbers of rows and columns, and/or sample size). In this case, we can obtain p-values by simulation (in R use argument \texttt{simulate.p.value = TRUE} in larger than 2x2 tables). Instead of examining exhaustively all possibilities, we repeatedly simulate outcomes (samples) under the null hypothesis and sampling model (fixed row and column margins) using a Monte Carlo procedure; for each such simulation we compute the statistic and then count how often the simulated statistics are larger than the observed one.
...
%% In R, r2dtable does that random generation.
...
First, this is the usual Fisher's exact test (and this you can get from R commander menus). I've added also the chisq.test.
<<>>=
fisher.test(Hx_tab)
chisq.test(Hx_tab, correct = FALSE)
@

Now, Fisher's with simulation of p-values. We use a small number here, the default of 2000 (for real, I'd use a lot more)\footnote{Incidentally, the p-value you are likely to get for this example is 0.0005, actually 0.00049975, which is \(1/(B + 1)\); this is because the pattern we observe is the most extreme of all the simulated ones, and we compute the p-value in the usual way: how many are as extreme as, or more extreme than, what we find. Since we have used 2000, the denominator is 2001. This is just a footnote, and you can skip it.}:
...
This is an interesting case, because of the small expected cell counts. \citet{bilder2024} discuss it in detail in pp.~180 and ff.\ (and they use a simulation approach, described in p.~182 and section 6.2.2, but a more sophisticated one, that we won't cover here; regardless, the p-value is obtained with a sampling model similar to the one in  \texttt{simulate.p.value}\footnote{Or, at least, that is my understanding of what they do, as they seem to condition on the margins: in p.~182 they write: ``(..) we can use one call to rmultinom(), where we set each \(\pi_{ij}\) (prob argument) to \(n_{i+} n_{+j}/n^2\) (the estimated expected count under the null hypothesis divided by the sample size). We chose to use the one multinomial model  here (...)''}).


\subsubsection{Issues with Fisher's exact test}
\label{sec:issues-with-fishers}

The main concerns with the use of Fisher's exact test in the literature  (when maybe your chi-square approximation is not that bad after all) are:
\begin{itemize}
\item Conservatism.
\item The conditioning on row and column margins, which is not relevant for most designs (where only one, or no margins, are fixed).

\end{itemize}

By the way, you might see this test referred as the ``Fisher-Irwin'' test.

Conservatism can be diminished using the \texttt{mid P-value}. This is well explained in \citet[section 2.6.3]{agresti2018} and in \citet[pp.~458 and 586 of]{bilder2024}.  Regarding the conditioning in both margins, some authors make a big deal of this, some others don't. This is a technical thing we cannot discuss in class. We just mention it here, since you might run into it. Of the textbooks cited here, \citet{agresti2012} discusses this in section 3.5.7 (p.~94 and ff.).



Why mention this? Some papers, most notably the widely cited  \citet{campbell2007} (see also \citealp{richardson2011, busing2016}) have argued that, for the 2x2 table, the usual recommendations (like those given above) for when not to use the chi-square test are too conservative and that the chi-square test could work well as long as the minimum expected number is at least 1 (a small correction needs to be made to the statistic; see \citealp{campbell2007,busing2016})\footnote{This issue is also mentioned in these on-line discussions: \Burl{https://stats.stackexchange.com/questions/100976/n-1-pearsons-chi-square-in-r/439226\#439226} regarding how to apply the correction itself, which includes a citation to the \citet{busing2016} paper;
\Burl{https://stats.stackexchange.com/questions/159057/alternatives-for-chi-squared-test-for-independence-for-tables-more-than-2-x-2/159063\#159063} and \Burl{https://stats.stackexchange.com/questions/14226/given-the-power-of-computers-these-days-is-there-ever-a-reason-to-do-a-chi-squa} about the minimial requirements to do a chi-square on a 2x2 table. If you are web surfing, you might as well read this great answer by Gordon Smyth (who has made many key contributions to the analysis of omics data) about why the conditionality issues are not such a concern: \Burl{https://stats.stackexchange.com/a/441146}.}. So the idea is that following the ``traditional'' recommendations (and using Fisher's exact test when not really needed) we are unnecessarily losing power\footnote{\citet{campbell2007} also suggest using a specific version of Fisher's test, which he refers to as the ``Fisher-Irwin'' test; the procedure he recommends is the one that R implements for two-sided tests.}.

\paragraph{Any practical suggestion?}\label{sec:any-pract-sugg}

If you do not want to read the primary statistical literature or consult with a statistician (a statistician  that knows about this stuff) then the path of least resistance is probably to stick to the ``usual rules'' and use Fisher's test if your data do not fulfill the ``usual rules''.  This is not a cynical recommendation, but is based on fairly standard and recent textbooks making these kinds of recommendations. Moreover, many people deal with tables larger than 2x2. But then, maybe a few years from now  we will be teaching something different because Campbell's suggestion will have become the standard approach for 2x2 tables.  % Myself \ldots I might give Campbell's procedure a try, but then I am often more concerned about p-values that are smaller than they should be than



\subsubsection{Chi square with \texttt{simulate.p.value}}

This is another alternative: just use your usual \texttt{chisq.test} but with option \texttt{simulate.p.value = TRUE}. No reliance in the asymptotic approximation. There is catch, though: in R, when you use \texttt{simulate.p.value}, the simulation uses the same sampling model as in Fisher's exact test\footnote{This is copied verbatim from the help of \texttt{chisq.test}: ``In the contingency table case, simulation is done by random sampling from the set of all contingency tables with given marginals (...). (...)  Note that this is not the usual sampling situation assumed for the chi-squared test but rather that for Fisher's exact test.''}. There is nothing wrong with this, but if you have concerns about Fisher's test, this simulation procedure carries the same concerns. But this might be a sensible alternative to using Fisher's test anyway. See a detailed example in \citet[sections 6.2.1 and 6.2.2 and p.~182]{bilder2024}: they show (in the diet example) how Fisher's might be conservative relative to \texttt{chisq}, including \texttt{chisq.test} with \texttt{simulate.p.value = TRUE}, and they explain in detail how to carry out the permutations, and discuss (p.~182) choosing the sampling model.
...
More sophisticated simulation schemes are possible, but that is beyond the scope of this class (and might run into other problems, like generating marginals that are 0, etc). In R, package \texttt{exact2x2}, \Burl{https://cran.r-project.org/web/packages/exact2x2/index.html} might be of interest.



(And, if you are curious, file \texttt{chisq-simulate-fisher-p-values-differ.R} contains a long exploration of differences in p-values from Fisher's and \texttt{chisq.simulate} with examples. This is, of course, supplemental material that you can ---and probably should--- ignore.)
...
These two could be better plots, though:
...
Interestingly, however, the last plot does not seem to indicate the very strong differences we find.  You might want to look at the residuals we discussed in section \ref{sec:residuals} and see if you can relate them to the plots. Moreover, what is the size of the difference that is being detected?
...
of the categorical data. % (see figure \ref{fig:Visualization-of-multivariate})
 Unfortunately they are  not implemented in R commander.
...
<<>>=
library(vcd)
mosaic(xtabs(~ Effect + HRE, data = Hx),
       shade = TRUE)
@

% %%% We split along the Effect axis, then
% %% in each row. We can change it doing
% <<>>=
% ## transposes, as HRE now on y-axis
% ## but split is vertical
% mosaic(xtabs(~ Effect + HRE, data = Hx),
%        shade = TRUE, direction = "v")
% ## or transpose: HRE on y axis, but horizontal
% mosaic(xtabs(~ HRE + Effect, data = Hx),
%        shade = TRUE)

% @


Wait!!! What residuals are those??
...
We can use the standardized residuls with the code below (this is the same procedure as used by \citealp{agresti2018} in p.~41 and by \citet{kateri2014} in pp.~56 and 138).
...
% Fourfold plots are very useful to represent 2x2 (or collections of 2x2, such as )

% In fourfold plots, the cell


% the pattern of association, a visual indication of whether the test is significant or not
% <<>>=
% ex_tab <- xtabs(~Drug+Outcome, data=Exercise3)
% fourfold(ex_tab)
% @


\citet[in section 2.4]{kateri2014} provides a great and brief overview of plots for categorical data, including several ones we have not discussed (most notably the fourfold plots and sieve diagrams); this book is available from UAM's library.
...
%% Xu is OK, but Kateri or Friendly and Meyer a lot better.


\section{Analysis of two-way tables: some salient things we have left out}\label{sec:Other-test-for}


Be aware that we have only scratched the surface of categorical
data analysis and of the analysis of two-way tables. There are many other questions/issues/procedures that can be relevant for categorical data:
\begin{enumerate}
...
%% FIXME: for future years, see how we do with time, and maybe uncomment this code?
...
%% FIXME: this might be uncommented too? And discuss Simpson's paradox?
...
\item Some categorical variables are actually ordinal, not simply nominal. In this case, it is possible to conduct more powerful tests that account for the ordinal nature of the variable. (One simple case is shown in section 8.3 of \citealp{Dalgaard2008}; for a lot more detail, see the excellent books by Agresti, \citealp{agresti2012},  \citealp{agresti2018}, Kateri, \citealp{kateri2014}, or Bilder and Loughin \citealp{bilder2024}).
...
We spent time covering linear models. Generalized linear models extend linear models for responses that are not necessarily numerical, or that can be numerical but are not at all Gaussian. Briefly, generalized linear models allow us to put, on the right of an expression like ``\verb@y ~ x + u + ...@'' \  a random variable \(Y\) that can be a proportion, for example.

A common example is logistic regression, where we model, on the left of the ``~'', the probability of an event. So we can model the probability of response (healed vs.\ non-healed) as a function of a bunch of variables. The simplest generalized linear models reduce to a contingency table, but generalized linear models allow us to model much more complex dependencies. We can have additive effects, interactions, etc. Moreover, they provide estimates of effects and the same principled ways of addressing modeling questions we say for linear models.  Generalized linear models can also be used to model categorical data that fall into more than two categories (multinomial data) or counts (Poisson data). Etc, etc. These topics are covered by \citet{kateri2014}, \texttt{agresti2012} \citep{agresti2018}, \citet{bilder2024}, and \citet{fox2018} and \citet{faraway_extending_2016} (even if the last two do not explicitly have ``categorical'' in their title).
...
\item  Bar graphs are appropriate to represent a single categorical
variable, whereas stacked bar plots and mosaic plots are used to represent multivariate categorical data.
\end{enumerate}
...
%% I don't understand: this sounds more like compariing if the number of genes in each strand differs.
...
Much of what we have covered here is discussed in \citet{Dalgaard2008} and \citet{diez2019}. The first is available through UAM's electronic library, and the second is available in several formats, from their web page. \citet{kateri2014} covers (in just the first two chapters) all we have discussed here (and a lot more); it is also available from UAM's electronic library.

Beyond that, the first place I always go to for issues related to categorical data analysis is \citet{agresti2012} and \citet{agresti2018}. \citet{bilder2024} is also a great book. For generalized linear models with R, \citet{fox2018} and \citet{faraway_extending_2016} are very good.
...
You can enter the tabulated values directly, and do a test. Go to Statistics \(\rightarrow\) Contingency
tables \(\rightarrow\) Enter and analyze... select the appropriate number of rows
and columns and enter the values. Finally select the test(s) you want
to run (see figure \ref{fig:Entering-contingency-tables}).
