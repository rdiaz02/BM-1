...
0.05$.  You will see more about multiple testing in the final lesson but
if the reason why we need this here is not clear, ask it in class
\textbf{now!}.
...
two factors, Diet and Drug, we do not need to transform them. Call the
data \Robject{dcholest}.
...
SPSS, SAS, etc.)\footnote{The type of sums of squares used (and,
  ...
  \Burl{https://stat.ethz.ch/pipermail/r-help/2006-August/111927.html}. Please
  beware
\label{fn:2}}.
...
\item Added variable plots and ``Component+Residual'' (or partial
  ...
  deviations from linearity and could suggest appropriate transformations.
...
later\footnote{R.\ Fisher, one of the fathers of modern statistics, and
  modern quantitative and evolutionary genetics, is often quoted in this
  context because he said ``To consult the statistician after an
