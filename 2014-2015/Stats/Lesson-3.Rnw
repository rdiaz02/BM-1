...
Import \Robject{MIT.txt}. These are data about mitochondrial activity
related to three different training regimes. Whoever coded the data,
however, used a number for ``training'', which is misleading, because this
...
name the model as \Robject{AnovaMIT} (entering that in ``Enter name for
model'')
...
an ANOVA. There is nothing wrong with data (and there is a simple
...
transform them. Call the data \Robject{dcholest}.
...
or, much nicer, using the \CRANpkg{RcmdrPlugin.HH} and going to ``Graphs'',
...
(one or the other might be easier to decode):
...
Given these results, it makes little sense to report any global main
effects and, in fact, we would rarely be interested in interpreting the
significance (or not) of the Diet or Drug term, by themselves, in the
presence of interactions. In general, \textbf{in the presence of
  ...
  in section \ref{orderfactors} for some entries and references.}.
...
\item Type II sums of squares (as well as t-statistics from a linear
  ...
  assuming interactions with the term in question are zero).
  
\item Type I (or sequential) sums of squares do not. They are sequential,
  in the order shown in the output.
\end{itemize}

Biologically, this makes a lot of sense. Think about it. And this, of
course, affects models with two, three, \ldots factors.\footnote{The type of sums
of squares used (and, actually, the type of hypothesis tested) is a
debated topic. The book by John Fox ``Applied regression analysis and
generalized linear models'' contains a great discussion (his book with S.\
Weisberg ``An R companion to applied regression'', one of the recommended
books, does too). An email-length discussion of these topics can be found
here \Burl{https://stat.ethz.ch/pipermail/r-help/2006-August/111854.html}
and \Burl{https://stat.ethz.ch/pipermail/r-help/2006-August/111927.html}
}
...
Read the full data and call it anage\_a\_r (aves, reptilia). 
...
would not know this before hand, but I do, so create those new
variables)\footnote{Creating these new variables is not really necessary
  in general for fitting models. But some functions from the
  \CRANpkg{HH} lead to problems if we don't.}. It is much faster to
just do it by typing the code in the R Script or RStudio console:
...
and call it anage\_a (yes, you know how to do that; look at Figure
\ref{subset}).
...
are going to do is not really correct, as the data are not really
independent (species share common ancestors, and they are related in
varying degrees); this is just for the sake of the example, and because
they are a nice set of data\footnote{This can be done correctly,
  incorporating phylogenetic information, but this is way out of the scope
of this class}.
...
plot'' (you might want to unclick the ``Show spread''\footnote{I find the
  spread information to be confusing, at least to me. But I like to leave
  the smoothe line, as it can help me see errors in the model specification.}):

<<out.width = '10cm', out.height = '10cm'>>=
...
field of allometry and life history suggest that is a reasonable way to
go).
...
the R Script/ RStudio console, giving it a new name (\Robject{metablog}). We will also replot,
and pay attention because in the scatterplot menu you can tell it to log
both axis.
...
<<>>=
scatterplot(Metabolic.rate..W.~Body.mass..g., log="xy", reg.line=lm, smooth=TRUE, spread=FALSE, 
  id.method='mahal', id.n = 2, boxplots='xy', span=0.5, data=anage_a)
@ 
...
now we can have two or more independent variables. I will a dataset that
is small set of the original \Robject{cystfibr} data set from package
\CRANpkg{ISwR} (by Peter Dalgaard, and prepared to go along the book
...
You should be able to interpret all output without problems. Can we add
interactions? Yes, of course. Interactions between continuous variables,
however, are harder to interpret (they represent curved surfaces, whereas
an additive model is just a plane ---or hyperplane)\footnote{If you want
  to play around with a regression plane or regression surfaces, go to
  ``Graphs'', ``3D graph'', ``3D scatterplot''. In options, you can choose
  a plane or three different surfaces ---you might need to play with the
  degrees of freedom if you get errors. You can zoom in and out
  with the mouse wheel and move/rotate the figure. Try doing that during
  your TFM defense! Of course, that allows only for up to one dependent
  variable and two independent ones. Our brains do not seem ready for 4D
  and higher.}.
...
between explanatory variables, and ``the information of age relevant for
predicting pmax is already contained in the height and weight'' (in fact,
that age, height, and weigth are correlated is easy to check with a
scatterplot matrix).
...
Variable selection is a touchy and delicate subject. We will summarize it
as follows: please, distrust automated variable selection procedures that
rely on p-values or F-statistics of individual variables (in all their
variants, such as stepwise, etc). Much more reasonable are strategies
based on criteria such as AIC and even better is bootstrapping the whole
process to get estimates of error, predictive ability, etc. Even better,
of course, is subject-matter guidance on how to proceed and what are and
are not candidates for deletion and in what order. The book by Frank
Harrell, ``Regression modelling strategies'' contains great discussions of
these topics. The actual data set used here is discussed, also in the
context of variable selection, by P.\ Dalgaard in chapter 11 of
``Introductory statistics with R''.
...
There is no evidence of interaction. But, what would an interaction have
looked like? Different slopes:
...
models where we have no evidence of different slopes, but evidence of
different intercepts: these means parallel lines.



\subsection{ANCOVA with the birds and the reptiles}\label{ancova_rept}
We will use the data we used in section \ref{regr}, but now the full one:
\Robject{anage\_a\_r}. We will go pretty fast here (this is just a kind of
review), and will examine two things:
\begin{itemize}
...
Again, these analyses are not fully correct as we ignore phylogenetic
relatedness. But they are nice to illustrate a couple of points. We will
directly use log transformations (again, theory and previous empirical
evidence indicate this is the way to go).


First, metabolic rate vs.\ body mass:
<<>>=
metab_b_r <- lm(logMetabolicRate ~ logBodyMass * Class, 
                data = anage_a_r)
summary(metab_b_r)
@ 
...
metabolic rates scales with body mass the same way in an endothermic group
(birds) and a ectothermic one (reptiles), but their rates are not the same
(birds' are higher).
...
<<>>=
longev_b_r <- lm(logLongevity ~ logBodyMass * Class, 
                data = anage_a_r)
summary(longev_b_r)
@ 
...
\item Constant variance (across groups or over the range of the independet
  variables). Often referred to as ``homoscedasticity''.
...
the data we have been using, anyway). Lack of independence among data is
the reason why the analysis with the AnAge data set are not really
correct.
...
\item[Fitted value] The predicted, or fitted value: if we have an equation
  ...
  the $Y$ for the observe combinations of $X$ and $Z$ (with the values of
...
The one on the upper left is used to judge if the functional form of the
...
homoscedasticity are present (though, right now, with so strong evidence
...
dotted line (with a bit of  allowance for deviations in the tails); things
...
Now, repeat the above with the mode that has taken logs:
<<>>=
oldpar <- par(oma=c(0,0,3,0), mfrow=c(2,2))
plot(metablog)
par(oldpar)
...
\Robject{metab\_b\_r} (section \ref{ancova_rept}), which are relatively
decent. 

\Robject{mcyst2} (section \ref{ancova}) and \Robject{mcyst} (section
\ref{multreg}), the two models we fitted to the cystic fibrosis data, both
look relatively decent, although there could be some concerns about
increases in variance with fitted values.  However, it is not easy to
tell, because of the relatively few points. This, of course, makes sense:
if there are few points, we will only be able to detect if a model is a
bad one if it really is very bad.
...
\Robject{AnovaModel.1}, from section \ref{oneway}, looks just fine.
...
\item Added variable plots and ``Component+Residual'' (or partial
  residual) plots allow us to examine, in models with multiple regressors,
  deviations from linearity and appropriate transformations. 
...
\item Added variable plots are particularly useful in multiple regression
  problems with multiple independent variables; they can help identify
  influential points (which are easily masked with multiple variables) and
  to try to find a good functional relationship.
...
\item Package \CRANpkg{car} and the accompanying book ``An R companion to
  ...
  summary, at the very nice sections 83 to 8.5 in Kabakoff's ``R in
  action'.'
\end{itemize}
...
This also relates to questions we asked in Lesson 2: What if we had
...
samples? Should we start each one of the four different diets in different
days of the week, or should we have similar sized groups of each diet
starting every day of the week? Do you give treatment A to all even
numbered samples and treatment B to all odd numbered ones, or do you
randomize order? Etc, etc.
...
half-page comment about this by G.\ Churchill in \textit{Science}, 2013,
v.\ 343, p.\ 370). In fact, some mistakes made during the experimental
design phase just cannot be corrected later.
