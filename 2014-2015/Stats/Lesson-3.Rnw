...
or, much nicer, using the ``RcmdrPlugin.HH'' and going to ``Graphs'',
...
presence of interactions. In general, \textbf{in the presence of
  interactions, we generally refrain from interpreting main
  effects}\footnote{Properly formulated, and probably using other types of
contrasts ---contr.sum--- marginal tests in the presence of interactions,
what are called Type III, can make sense, but are not always of interest.}. 
...
now we can have two or more independent variables. I will use the dataset
``cystfibr'' from package ``ISwR''.
...
Look at the help for the dataset. I will use a simplified example here,
where we model
...
<<>>=
mcyst <- lm(pemax ~ age + height + weight, data=cystfibr)
summary(mcyst)
@ 
...
%% <<>>=
%% print(scatter3d(pemax~age+height, data=cystfibr, 
...
of squares (when entered first and using anova, so Type I) but not when we
test it after all other terms in the model (Type II). Why? One possible
explanation is that there is correlation between explanatory variables,
and ``the information of age relevant for predicting pmax is already
contained in the height and weight'' (in fact, that age, height, and weigth
...
scatterplotMatrix(~age+height+pemax+weight, reg.line=lm, 
                  ...
                  diagonal = 'density', data=cystfibr)
@ 
...
<<>>=
anova(lm(pemax ~ height + weight + age, data = cystfibr))
@ 

And similar if you place weight first.
<<>>=
anova(lm(pemax ~ weight + height + age, data = cystfibr))
@ 
...
it with the cystic fibrosis data set.  In terms of terminology, ANCOVA
(which is still used, but not as widely as it used to be 20 years ago),
refers to a model where we have a discrete and a continuous variable as
...
<<>>=
mcyst2 <- lm(pemax ~ age * sex, data=cystfibr)
summary(mcyst2)
@ 
...
scatterplot(pemax~age | sex, reg.line=lm, smooth=FALSE, 
            spread=FALSE, id.method='mahal', id.n = 2, boxplots='xy', 
            span=0.5, by.groups=TRUE, data=cystfibr)
@ 
...
One can visualize this also  with the function ``ancova'' in package
``HH'' (which is loaded when we load ``RcmdrPlugin.HH'', so we do not need
to load it again):

<<>>=
ancova(pemax ~ age * sex, data = cystfibr)
@ 
...
check some of their basic assumptions.
