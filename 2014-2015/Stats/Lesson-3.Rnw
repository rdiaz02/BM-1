...
<<>>=
dchol <- read.table("Cholesterol.txt", header = TRUE)
@ 
...
cholanova <- (lm(y ~ Diet*Drug, data=dchol))
## This shows the ANOVA table. Notice the "Type II"
Anova(cholanova)

## Now we are show the 3 by 2 table of means, standard deviations, and number 
## of observations
tapply(dchol$y, list(Diet=dchol$Diet, Drug=dchol$Drug), mean, na.rm=TRUE) # means
tapply(dchol$y, list(Diet=dchol$Diet, Drug=dchol$Drug), sd, na.rm=TRUE) # std. deviations
tapply(dchol$y, list(Diet=dchol$Diet, Drug=dchol$Drug), function(x) sum(!is.na(x))) # counts
@ 
...
<<>>=
trellis.device(theme="col.whitebg")
plot(allEffects(cholanova), ask=FALSE)
@ 
...
<<>>=
with(dchol, interaction.plot(Drug, Diet, y, type = "b"))
@ 
...
Which means differ with covars
