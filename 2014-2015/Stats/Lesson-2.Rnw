...
took BM-1, you are in charge of looking at the data and answering the
...
information to more information'' we can organize them this way:
...
\item[Ordinal scale] The data can be ordered in the sense that you can say
  ...
  shrimp > liver''. You might assign the value 1 to chocolate (your most
...
\item[Interval or ratio scale] You can take differences and ratios, and
  they do have meaning\footnote{Some authors make a distinction between
    ...
    scales have a natural zero.}. If a subject has a value of 6 for PTEN,
  another a value of 3, and another a value of 1, then the first has six
  times more than the last, and two times more than the second.
  
\end{description}


We will try to be careful. Nominal data we will always try to keep as
``things without numbers'', so that we make no mistakes (i.e., keep the
aminoacids names, not just a bunch of 1 to 20). Ordinal scale are
trickier, and we will need to think about the type of data and the analyses.
...
We first need to import the data (from ``Data'', ``Import data''). Make
sure you name it sensibly; for instance, dp53. When you do that, you will
see something similar to (not identical! because I did it suppressing
unneeded stuff):
...
For all except the trivially tiny datasets we want to use graphics. 
...
``Submit''. %% (It is simpler if you copy, then clear, then paste and modify).
...
generate the HTML report. That is and HTML file you can put on a web page,
send around, etc.
...
(We will spend time in class making sure all this is understood).
...
(e.g., section \ref{pseudorep}). Lack of independence is a serious and
pervasive problem (and one form of non-indpendence is also referred to as
``pseudoreplication'' \footnote{A major paper, a long while ago, in
  \textup{Ecological Monographs} by Hurlbert dealt with this and made
  ``pseudoreplication'' a well known term.}).
...
close'' we worry mainly about asymmetries (that tails are heavier is
rarely a concern). And things tend to get ``sufficiently close'' as sample
size grows larger (a consequence of the central limit theorem); how large
is large enough? Oh, it depends on how far the distribution of the data is
from the normal distribution, but often 10 is large enough, and 50 is
generally well large enough (but in certain cases 100 might not be even
close to large enough).

Of course, although this should be obvious: when we talk about symmetry
we are talking about the data distribution of \textbf{each group
  separately}. 
...
by boxplots). In general, points very far from the rest of the points will
have severe effects on the value computed for the mean (not the median
---this is also related to why nonparametric procedures can be more
robust). What to do, however, is not obvious. An outlier might be the
consequence of an error in data recording. But it might also be a
perfectly valid point and it might actually be the ``interesting
stuff''. Sometimes people carry out (and, of course, \textbf{should
  explicitly report}) analysis with and without the outlier; sometimes the
same qualitative conclusions are reached, sometimes not. Please, think
carefully about what an outlier is before proceeding with your analysis
but do not get into the habit of automatically getting rid of potential
outliers. And whatever you do, you should report it.


The book by Ruper Miller ``Beyond Anova'' (Chapman and Hall, 1997)
contains a great discussion of these issues.
...
Let us look at the boxplot for ``pten''. What does it look like? Take the log
transformation: create a new variable. (Yes, find this out).

Now, plot pten and its log (a boxplot). And do t-tests of the original and
...
We cannot do a paired t-test directly with our data. We first need to
reshape the data as R Commander ---in contrast to using directly R--- will
not allow us to carry out a paired t-test on these data as they are.cc


We want the values of myc of the same subject to be on the same row. This
is what we will do:

\begin{enumerate}
\item Create a data set that contains only the values for observations
  with status ``Cancer''. (``Subset'', under ``Active data set''). Call
  this, say, cc. 
...
\item We will see below a way to accomplish the same with a linear
  model. And no reshaping needed.
\end{itemize}
...
You can do this in a simpler way using the RcmdrPlugin.doBy (BEWARE: you
must install and use the version of RcmdrPlugin.DoBy I
provide\footnote{There is a bug in one of the functions in the original
...
\red{FIXME!zz: make sure I provide the zip file for windows!}
...
\item For each of the new data sets, turn the id column into the rownames
  ...
  names''.
\item Merge the two new data sets, again using ``Merge columns''. Call
  this m3, for instance.
\end{enumerate}
...
<<>>=
t.test(m3$diff.nc.c, alternative='two.sided', mu=0.0, conf.level=.95)
@ 
...
plotMeans(dmyc$myc, dmyc$cond, error.bars="se", 
          ylab = "MYC", xlab = "Condition")
@ 
...
The cleanest thing to plot? A boxplot (or stripchart if few points) of the
within-individual (or Intra-subject) differences. In fact, I will now
create them from scratch directly from R (this \textbf{assumes} that the
data are ordered the same way by patient: the first myc.cancer is the same
patient as the same myc.nc, etc. This is the case in our data, but need
not be; see details in section \ref{ptr})



<<fig.width=5, fig.height=5, echo=TRUE>>=
diffs <- dmyc$myc[1:12] - dmyc$myc[13:24]
Boxplot( ~ diffs, xlab = "", ylab = "Intra-subject difference in MYC")
@ 
...
\subsection{A first taste of linear models}

So we implicitly have a model like this:
...
\begin{figure}[h!]
 \begin{center}
 \includegraphics[width=0.80\paperwidth,keepaspectratio]{lm-myc.png}
...
``should I collect data so that data are paired or not?'' A paired test
...
really are subject effects. But not always.
...
\item What about the relative efficiency of the non parametric procedure?
  (the sample size required for two different procedures that are equally
  good in power and type I error rate). When assumptions are met,
  parametric methods do have more power (though not always a lot
  more). When assumptions are not met, nonparametric methods might have
  more power, or the correct Type I error, etc. Note that very, very, very
  small p-values are often not achievable with non-parametric methods
  (because of the way they work), and this is a concern with multiple
  testing adjustments.
...
of the ranks of one of the populations is larger (or smaller) than the
...
and it does not for ordinal data). These differences (discarding the sign)
are then ranked, and then sum of the ranks of all the positive differences
is computed and compared to the null distribution.
...
<<>>=
wilcox.test(m3$NC.myc, m3$Cancer.myc, alternative='two.sided', paired=TRUE)
@ 
...
differences (this test is not directly available from R Commander)
...
How likely we are to detect a difference that really exists depends on:
...
Do you understand each one of these?
...
\begin{figure}[h!]
 ...
 \caption{\label{power} Playing with power, from the
   RcmdrPlugin.TeachingDemos, Power of the test.}
 \end{center}
 \end{figure}



There are simple, standard ways of
figuring out the power. But they require you to specify the above
values. Not always known (or easy to guess). The ``RcmdrPlugin.IPSUR''
package provides some extra tools.


\begin{figure}[h!]
 \begin{center}
 \includegraphics[width=0.60\paperwidth,keepaspectratio]{ipsur-power.png}
 \caption{\label{ipsurpower} Entry window for RmcdrPlugin.IPSUR: Power for
   t-test, under ``Statistics'', ``Power (IPSUR)''.}
 \end{center}
 \end{figure}
...
\section{Non-independent data: multiple measures per
  subject}\label{pseudorep}
...
carefully. It looks like there are \Sexpr{nn} observations per
subject. Basically, each subject has been measured \Sexpr{nn} times. Thus,
there are not \Sexpr{nn * (n1*n2)} independent measures. Again, ask
yourself: what is the true ``experimental unit'' (or observational unit)?
It is arguably the subject, which is the one that has, or has not, cancer
in this case. The \Sexpr{nn} measures per subject are just that: multiple
measures of the same experimental unit. This is an idea that should be
crystal clear; make sure you understand that.
...
<<>>=
aggbrca <- aggregate(dbrca[,c("brca2"), drop=FALSE], by=list(cond=dbrca$cond, 
  id=dbrca$id), FUN=mean)
@ 

We want to double check that we have the exact same number of measures per
subject. How? A simple thing is to aggregate again (give the output
...
difference. When we do them incorrectly, we can end up believing that
...
\begin{itemize}
\item A total of two female mice were used. One was given food with DHA
  one without. They were fed the specified diet, and then they mated and
  got pregnant.
...
side in the animal room or how nice the male was or \ldots).
...
There are some recent papers about issues like this (e.g., Lazic, 2010,
\textit{BMC Neuroscience}) and this is probably a pervasive (but difficult
to detect) problem. This kind of meaningless analysis can lead to lots of
non-reproducible results.

This leads us to the fundamental idea of \textbf{thinking carefully about
  the experimental design}, something we will take a quick look at in
Lesson 3.
...
aneuploidy in embryos from two groups of fathers, ``younger fathers''
and ``older fathers'' (wait, not older, just ``fathers that so far have
...
problem. A simple approach, assuming each father contributes the same
...
Of course, this \textbf{crucially assumes} that the data are ordered the same way by
patient: the first myc.cancer is the same patient as the same myc.nc,
etc. This is the case in our data, but need not be. We can check it:
