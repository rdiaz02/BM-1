...
op <- par(mfrow = c(3, 2)) ## to show 2 by 2
...
referred to as ``pseudoreplication'' \footnote{A major paper, a long while
  ago, in \textup{Ecological Monographs} by Hurlbert dealt with this and
  made ``pseudoreplication'' a well known term.}).
...
Boxplot( ~ diff.nc.c, data = m3, xlab = "",
        ylab = "Intra-subject difference (C - NC)", main = "MYC")
@ 
...
\item What about the relative efficiency of the non parametric procedure?
  ...
  testing adjustments in omics experiments (this will be covered in the
  last lesson).
...
incorporate this nestedness. A very recent 2-page paper is Blainey et al.,
2014, ``Points of significance: Replication'', in \textit{Nature Methods},
2014, 11, 879--880, where they discuss issues of sample size choice at
different levels.
