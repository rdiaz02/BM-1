...
##                justDoIt(cat(paste("\n dsnameValue is ", dsnameValue, "\n")))
    ...
    if (dsnameValue != "") {
      ...
      ## justDoIt(command) ## RDU
      doItAndPrint(command) ## RDU
      ...
      for (i in 1:length(names(get(dsnameValue)))) {
          ## command <- paste(dsnameValue, ".", names(tempsp)[i], " <- tempsp[\"", names(tempsp)[i], "\"]", sep="") ## RDU
        command <- paste(dsnameValue, ".", names(tempsp)[i], " <- ",dsnameValue ,"[\"", names(tempsp)[i], "\"]", sep="") ## RDU
        doItAndPrint(command)
        
    }
      ## next loop by RDU: make sure they are data frames, so we can use them
      for (i in 1:length(names(get(dsnameValue)))) {
          command <- paste0(dsnameValue, ".", names(tempsp)[i],
                            " <- as.data.frame(",
                            dsnameValue, ".", names(tempsp)[i] ,")")
          doItAndPrint(command)
      }
      ...
      for (i in 1:length(names(tempsp))) {
        ## command <- paste("tempsp[\"", names(tempsp)[i], "\"]", sep="") ## RDU
        command <- paste("SplitList[\"", names(tempsp)[i], "\"]", sep="") ## RDU
        doItAndPrint(command)
      }
    }
