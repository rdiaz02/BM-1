...
This work is Copyright, \copyright, 2014-2025, Ramon Diaz-Uriarte, and is
...
%% See notes in PRSTR
%% ~R-basic-stats/R-basic-stats.Rnw
...
Back to our Anova.  To carry out all pairs of tests and control for multiple testing, fit the ANOVA again, but now make sure to click on ``Pairwise comparisons
...
\subsubsubsection{Side note: Interpreting confidence intervals}
...
%% FIXME: this is kinda confusing. Copy this section from PRSTR.
...
\item When there are interactions, this is more complicated. For example, at what
  ...
  very simple numerical example illustrating the problem is shown below in \ref{ci-two-way-int-example}.
