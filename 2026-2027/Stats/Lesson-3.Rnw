...
an ANOVA. There is a simple
correspondence between a t statistic and an F statistic (and this is the kind of t-test where we assume the variance of the two groups is equal).
