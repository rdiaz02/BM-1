...
This work is Copyright, \copyright, 2014-2026, Ramon Diaz-Uriarte, and is
...
Linear models and their extensions (which include logistic regression, and
...
We can try a boxplot overimposing the observations (and, if you want, you can also try a violin plot) using \texttt{RcmdrPlugin.KMggplot2}, as we have seen in Lessons 1 and 2:

<<>>=
require("ggplot2")
.df <- data.frame(x = dmit$ftraining, y = dmit$activ)
...
Of course, you don't need R Commander: you can get a similar thing building what you want from
...
if you compare all pairs of means you carry out ${K \choose 2} = \frac{K
  (K-1)}{2}$ tests). Here, we will control the family-wise error rate \textbf{(FWER)}, the
...
(\nameref{sec:mult-comp-fdr}). For now, remember, we are focusing on controlling the FWER: the probability of rejecting one or more null hypotheses when we shouldn't.




Since understanding what we just said is crucial, we will use another two examples, a depressing and an uplifting one. If you understand them, you understand why we need multiple comparisons:
...
To carry out all pairs of tests and control for multiple testing, fit the ANOVA again, but now make sure to click on ``Pairwise comparisons
...
## numSummary(dmit$activ , groups=dmit$ftraining,
...
%% \subsubsubsection{The 95\% family-wise confidence interval}
...
Again, notice the output says ``simultaneous'': as emphasized in Lukas Meier's ``ANOVA and Mixed Models'' (2025, CRC Press; HTML from \Burl{https://people.math.ethz.ch/~meier/teaching/anova/contrasts-and-multiple-testing.html\#sec-tukey-hsd}):
``these confidence intervals are simultaneous, meaning that the probability that they all cover the corresponding true difference at the same time is 95\%.''.
%()%% because emacs gets confused with paren.
Notice the ``all cover the corresponding true difference is 95\%''; why?
Because: \(\mbox{FWER} = P( \ge \mbox{1 false rejection})  \le 1 - P(\mbox{all CIs cover the truth}) = 0.05 \)
%()%% because emacs gets confused with paren.
\footnote{Technical point: And why the last \( \le \)?
Because the FWER control has to work both under the complete null, when all nulls are true, but also when only some nulls are true. Under the complete null, when all nulls are true, we would write
\( P( \ge \mbox{1 false rejection}) = 1 - P(\mbox{all CIs cover the truth}\).
In the second case, when only some nulls are true,
\(P( \ge \mbox{1 false rejection})  < 1 - P(\mbox{all CIs cover the truth})\);
the CI for a truly false null might not cover its true value, and this would not be a Type I error. This is why on this second case, FWER is strictly less than \(1 - P(\mbox{all CIs cover the truth})\).}.
...
\end{itemize}
 ...
 Let's go back to the numerical output.
...
the mean of group 4,
or comparing specific treatments against a control. Etc, etc. We will not get into this. A very good introduction, in addition to other references already
...
Of course, you can just carry out any two-group comparison as
an ANOVA. There is a simple correspondence between a t statistic and an F statistic, and this is the kind of t-test where we assume the variance of the two groups is equal\footnote{It is possible to compare the means of two or more groups, in an ANOVA-like way, without assuming equal variances; see function \texttt{oneway.test}. More complex models, such as mixed-effects models and generalized least squares (e.g., \texttt{lme} and \texttt{gls} in package \texttt{nlme}), also allow variances to differ between groups, or to depend on other factors.}.
...
\subsection{Can you do an ANOVA with a sample size of 1 per group?}\label{anova-only-one}
Why or why not?
...
Note that, in our usage of Tukey, we did not pre-specify the actual $Pr(V \geq
...
% want more details, sing up for some of the other classes we teach :-) )
. But go back a couple of
...
In a two-way ANOVA, we are interested in the effects of two factors, not just one. For example, our experiment might involve simultaneously changing diet and drug.


A key issue in models with two or more predictor variables (such as two-way
ANOVA) are interactions. What is an interaction? The phenomenon of interaction should be familiar to you: it is very common in life in general, and in biology you might have previously seen at as epistasis in genetics; or take a look at the `Interactions'' section in the leaflet (prospecto) of any drug.
...
Again: we will not emphasize understanding the coefficients in this class; we just wanted to show them to you, so you can see we are actually estimating four things (in the previous example, we were estimating three) and how we measure the interaction. More details in \nameref{anovaaslm}, but those details \textbf{are not} needed.
...
## Now we are shown the 3 by 2 table of means,
...
The Anova table for the cholesterol data shows very strong effects of interactions. Make sure you understand why we say this. We now want to understand those interactions. We will use an  ``Effects plot'' to look at interactions. Go to the ``Models'' menu and do a ``Effects plot''
...
boxplot(y ~ Drug * Diet, data = dcholest,
        col = c("salmon", "gold"))
...
In section \qref{ordermatter-appendix} we provide more details and examples.  That is optional reading and not part of this course.
...
There is also an additional document in Moodle about \(R^2\), ``Residuals, R-squared and adjusted R-squared''; again, it is there just in case you want to look at it, but you do not need to read it.
...
The variance inflation factor, that we can compute with function \texttt{vif} estimates, ``the harm produced by collinearity'' (Fox \& Weisberg, ``An R companion to applied regresssion, 3rd ed'', section 8.8):
...
Variance inflation factors should be part of your standard tools. There are no hard cutoffs. First, any VIF larger than 1 indicates that, well, there is harm being doing by collinearity. Values larger than 10 are often regarded as very serious collinearity problems. But values larger than 4 already signal a problem: the standard error is twice as large as it would be without collinearity.

For terms with multiple degrees of freedom (factors with more than two levels, polynomials, splines, etc), a modified VIF needs to be used, That is provided by the \texttt{vif} function in \texttt{car}, too. See details in the help and in Fox \& Weisberg's book.



(Note: VIF is accessible from the R commander menus too, from ``Models'' -> ``Numerical diagnostics'')
...
Now a simple numerical example (again, skip if you want):
<<>>=
...
If you want some specific names for variables to draw the above figures, you can do as follows:
\begin{itemize}
...
(residuals and leverage) that, together, are part of Cook's distance. Cook's distance measures the effect of individual points on the fitted coefficients (values of Cook's distance larger than 1 usually indicate a possibly very influential point, but any point with a widely outstanding Cook's distance deserves a closer look).  The plot called ``Influence plots'' is very similar to this one. However, I often find it simpler to look at plots Cook's distance (function \Rfunction{cooks.distance}).
...
\item There is in CRAN a very interesting package, \CRANpkg{gvlma}
  ...
  that implements the methods in their paper from 2006 ``Global Validation of Linear Model
...
mcystL_r2 <- update(mcystL_r,
                    . ~ . -age:weight:sex - height:sex - age:weight)
Anova(mcystL_r2)
anova(mcystL_r2, mcystL_r)
@

\subsection{AIC was not the issue: this model should not be fitted this way}
A final example, with another type of data.
In fact, the moral of this example is not really AIC, but the problems of the fit:  make sure to read section
\nameref{wrong}, as this is not a sensible model as fitted.
...
\externalf A separate PDF is available for this topic, \myhref{covars-interpr-causal.pdf}. Please, read it: we will cover it on the last day assigned to Lesson 3.
...
% might want to add too examples like the one on pp.\ 364 and ff in
...
## Note what are the estimates of the effects of A and B:
## the mean deviation of the second level from the first:
## A2
...
mI2.c <- lm(prestige ~ income * women * education * type,
            data = Prestige.c)
...
m_1.c <- lm(prestige ~ type * income * education,
            data = Prestige.c)
m_2.c <- lm(prestige ~ type * income * education + women,
            data = Prestige.c)
