...
## Wrapping of the R output, so that it does not run past the right
## margin; see the comments in knitr-setup.R
...
\author{Ramon Diaz-Uriarte\\
  ...
  Madrid, Spain{\footnote{r.diaz@uam.es}} \\
...
This work is Copyright, \copyright, 2014-2026 Ramon Diaz-Uriarte, and is licensed under a \textbf{Creative Commons
...
\externalf Now, look at file \myhref{CI-p-value-examples.pdf}.


This might help understand the CI. It comes from p.~98 of the recommended book by Dalgaard (``Introductory Statistics with R, 2nd ed'', Springer). He refers to a 95\% CI and writes: ``This is a 95\% confidence interval for the true mean; that is, the set of (hypothetical) mean values from which the data do not deviate significantly. It is based on inverting the t test by solving for the values of \(\mu_0\) [\(\mu_0\) is the hypothesized true mean] that cause t to lie within its acceptance region.''

That wording is not contradictory with the usual ``A 95\% CI is something (a range of values) we construct following a procedure that guarantees that, if we were to repeat it many, many times, it would cover/capture the true mean (or the true difference in means, if comparing two groups) 95\% of the time.''

Both definitions are about the same object (the confidence interval). The first describes what that set of values is, the second a crucial property of the procedure we use to construct it.

The CI is not one test; it is the result of asking, for every candidate \(\mu_0\), ``would I reject this?'' and keeping the ones I would not reject. Or, to put it differently: the CI collects the answers to a whole family of tests, one for each possible $\mu_0$: the interval is the set of $\mu_0$ that survive.

The virtue of the first wording is that it is closer to what we often want: what values are compatible with what we observed?



(This is somewhat technical.) Notice that this ``compatibility'' is in a frequentist sense, \(P(\mbox{rare data}|\mu_0)\), not in a  Bayesian sense, \(P(\mu_0|\mbox{data})\). The frequentist sense compatibility, \(P(\mbox{rare data}|\mu_0)\) means that, if \(\mu_0\) were the true mean, data like ours would not be unusual; this is an assertion about the data given the parameter, not about the parameter given the data. (We mention Bayesian procedures briefly later.)

% I have mu = 4. I construct that CI and it goes from 5 to 6? What gives?
% Because what this is saying is, since it did not capture 4 (the
% hypothesized true mean), that something like that, a CI that did not
% capture 4, will happen 5 % of the time, or less.
% El estadistico no asume H0 = 4. t = (bar{X} - \mu)/se es un pivote: se
% distribuye como t_{n-1} sea cual sea la mu verdadera. El test de mu0 = 4
% usa el pivote con mu = 4; el CI usa el pivote con mu libre y lo invierte,
% es decir, corre simultaneamente el test para todos los mu0. Que 4 quede
% fuera dice exactamente que el test de mu0 = 4 rejected. No hay
% contradiccion; hay una familia de tests y tu miraste uno. Si los
% estudiantes no tienen el concepto de pivote, la inversion les parece
% magia; si lo tienen, es obvia.
...
\subsection{One-tailed, examples}

For the examples below, suppose, for the sake of the argument, that the denominator of the t-statistic is 1 (so we can just focus on the difference in means). For these examples, I'll assume the degrees of freedom are 100.


\subsubsection{Example 1: \(H_0: \mu_A \le \mu_B. \ \  \bar{X}_A = 3, \bar{X}_B = 1\)}

\begin{itemize}
\item \(\bar{X}_A - \bar{X}_B = 2 \)
\item The t-statistic is \(2\).
\item What is a deviation from the null that is as extreme as, or more extreme than, the observed one?
  \begin{itemize}
  \item We observed \(\bar{X}_A - \bar{X}_B = 2\).
  \item The null says \(\mu_A \le \mu_B\).
  \item A deviation more extreme is any t-statistic as extreme as, or more extreme than, \(2\): further in the direction of the alternative, i.e., larger values of the t-statistic.
  \item Thus, anything to the right of 2, i.e., to the right of the red line below.
  \end{itemize}

  <<onet-1, out.width='7cm', out.height='7cm', echo = FALSE>>=
  curve(dt(x, df = 100), from = -4, to = 4)
  abline(v = 2, lty = 2, col = "red")
  axis(1, at = 2, labels = "2", col.axis = "red")
  text(x = 3, y = 0.3,
       paste("p-value = ", round(pt(2, 100, lower.tail = FALSE), 3)))

  xs <- seq(2, 4, length.out = 200)
  polygon(c(2, xs, 4), c(0, dt(xs, df = 100), 0),
          col = "red", border = NA)
  @
\end{itemize}
...
\subsubsection{Example 2: \(H_0: \mu_A \le \mu_B. \ \  \bar{X}_A = 1, \bar{X}_B = 3\)}
Like the above, but now we flip the observed values (not the null).
...
\item What is a deviation from the null that is as extreme as, or more extreme than, the observed one?
  ...
  \item We observed \(\bar{X}_A - \bar{X}_B = -2\).
  \item The null says \(\mu_A \le \mu_B\).
  \item A deviation more extreme is any t-statistic larger than \(-2\): further in the direction of the alternative, i.e., larger values of the t-statistic.

 \item Thus, anything to the right of -2, i.e., to the right of the red line below.
  \end{itemize}

  <<onet-2, out.width='7cm', out.height='7cm', echo = FALSE>>=
  curve(dt(x, df = 100), from = -4, to = 4)
  abline(v = -2, lty = 2, col = "red")
  axis(1, at = -2, labels = "-2", col.axis = "red")
  text(x = 3, y = 0.3,
...
In this example, we know our p-value will be at least 0.5: we are asking for the area to the right of \(-2\), so we know we already have half of the area (to the right of 0), plus something else.



Technical point. The null is composite (\(H_0: \mu_A \le \mu_B\)) instead of just a single value (\(\mu_A = \mu_B\) so \(\mu_A - \mu_B = 0\)).  The p-value is computed at the boundary case because that is the value in the null that makes the observed result most likely (it gives the largest p-value over the null)

\subsection{Some issues with one-sided tests}

Suppose you do a two-tailed test and find \(\bar{X}_A > \bar{X}_B\) (i.e., the sample mean of group A is larger than the sample mean of group B). You get a t-statistic \(t_1\) with a p-value of 0.06. This means that the area under the curve that is larger than \(t_1\) is 0.03, and the area under the curve that is smaller than \(-t_1\) is 0.03. \red{Make sure you understand this!!! Draw a picture of the t-distribution right now and place, in that figure, \(t_1\)! . The examples above should help.}
...
%% FIXME: start again as in 2024-25, the flip around a single one-sided hypot. and then move to the 2 tests
...
What case would represent examining if the effects of the brand name drug and a generic are similar?
...
This also emphasizes that which is the null, even in non-bioequivalence testing cases, can make a whole lot of difference. For example, suppose an industry wants to dump chlorine in a river. Is it enough if they say ``I tested the null that mortality of fish does not change when I dump chlorine''? Well, probably not. The precautionary principle would suggest that we want strong evidence that mortality does not increase.
For example, we might want to say ``We will only allow you to dump chlorine in the river if there is strong enough evidence that such action will not cause harm, for instance, will not increase fish mortality by more than a given amount''.
Likewise, with drugs, it is not uncommon to want to examine if a new drug, B, is not inferior to an established drug, A. Drug B might be a good choice if it is not worse than A by some pre-specified threshold.  These are examples of (non-)inferiority testing. % (see also \cite{walker2019, schumi2011}).

We will not pursue these issues further. Just be aware that ``the usual hypthesis testing approach'' does not cover all cases you might be interested in.


% We can approach this problem as one of finding evidence against the (new null) hypothesis that things differ by more of the specified tolerance, in our case that 1\% increase in fish mortality.

% This is not something you can settle with p-values as we have used them. \red{Why??}
...
stripchart(c(x, y) ~ c(rep("x", 10000), rep("y", 10000)),
           vertical = TRUE,
...
<<>>=
wilcox.test(dmycWide$myc.NC, dmycWide$myc.Cancer,
...
\begin{enumerate}
\item Carry out a test for normality (I guess for each group separately if this is a two samples t-test ---testing for normality collapsing the two groups would be absolutely nonsensical).
...
\item They can have power to detect a minor and irrelevant deviation from
  normality (e.g., slightly heavy tails) that have no effects on a
  t-test. So with very large sample size, an a slightlye ``heavier tails than a Normal'', where a t-test is just fine, we might be mislead
...
Again, this is the type of ``cargo-cult statistics''\footnote{``going through the motions with scant understanding''; Stark and Saltelli, 2018. Cargo-cult statistics and scientific crisis. \textit{Significance}, 15. This applies to statistics the ``cargo-cult science'' idea from Richard Feynman.} that does no body any good. Think about your data, what you want to test (is it means? or is it something else?), what are reasonable assumptions (heavy tails, asymmetries, etc) and whether the deviations are likely to cause trouble. The above procedure is wrong, and it is no substitute for thinking.
...
<<>>=
t.test(brca2~cond, alternative='two.sided',
