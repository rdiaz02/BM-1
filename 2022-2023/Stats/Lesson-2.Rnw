...
This work is Copyright, \copyright, 2014-2022 Ramon Diaz-Uriarte, and is licensed under a \textbf{Creative Commons
...
turned into factors, so we ask for it. This is no longer necessary with new versions of R.
...
Let us no do a couple of plots by group. (You'll need to use the command line, since a former plugin, ``RcmdrPlugin.plotByGroup'', is no longer available):
...
Note that, instead of the last call, you can use the \texttt{Boxplot} function (see below).
...
\item The threshold we use for saying ``the means differ'' ($\alpha$ level, or
  Type I error). (Note: if this is not clear, remember to ask me to explain the differences between
  $\alpha$ level and Type I error and p-value and the usual table of errors, I and II, version decisions and truth).
...
What can we do? We want to invert the process. You would want a procedure for answering the following question: ``Is there strong enough evidence that, if chlorine has an effect, the effect is no larger than an increase in fish mortality of 1\%?''. This is like inverting the burden of proof: now we want evidence in favor of the null (or, more precisely, in favor of a hypothesis that says that things do not differ by more than a given, small, value). This can be done; in some cases it is relatively straightforward (comparing means), in many others it is not.

We will not pursue this any further. But, please, think of other examples of similar cases. Examples involving, for example, human disease and certain environmental changes, or secondary effects of drugs.
...
We now go over the file  \texttt{confidence-intervals-p-values-interpretation.pdf}.
...
\ref{reshapefinal}. The rest is left here so that you can refer to it when you need to do it with your own data, but, again, we will \textbf{not} cover it in class. You need to understand the source of the problem. The syntax for the solutions is not important (you have it here, and can refer to it when you need it).
...
I'll use \texttt{merged3}; you can use \texttt{merged} or \texttt{merged2}
or \texttt{merged4}
...
<<>>=
merged3$diff.nc.c <- with(merged3, myc.NC - myc.Cancer)
@ 


Now, do a single-sample t-test:
<<>>=
with(merged3, (t.test(diff.nc.c, alternative='two.sided', mu=0.0, 
                      conf.level=.95)))
@ 
...
%% FIXME: say not a test of means and often not even of medians!!!
...
%% Christensen, W. F., & Zabriskie, B. N. (2022). When Your Permutation Test is Doomed to Fail. The American Statistician, 76(1) https://doi.org/10.1080/00031305.2021.1902856
...
\ref{reshaping}) so I will reuse our \texttt{merged3} data set:


<<>>=
wilcox.test(merged3$myc.NC, merged3$myc.Cancer, alternative='two.sided', 
            paired=TRUE)
@ 
