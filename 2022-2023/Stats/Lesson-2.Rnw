...
This work is Copyright, \copyright, 2014-2021 Ramon Diaz-Uriarte, and is licensed under a \textbf{Creative Commons
...
certain things in R, produce reports, etc, so we will not spend any time on it
...
transformations %% (see also section \ref{transform})
. However, think about
...
\subsection{\red{Confidence intervals and p-values: a longer discussion}}
...
\subsection{\red{An analogy: should I take the umbrella?}}
...
\subsection{\red{The winner's curse}}

\red{When studies have low power, estimates of effects from tests that yield
...
\red{Here is a interesting and recent example where the winner's curse can be
...
class we will definitely cover section \ref{datashape} and read section
\ref{reshapefinal}.
