...
What are the bands? Make sure you understand the difference between the confidence interval and the prediction interval.
...
The following code shows in detail how $R^2$ and adjusted $R^2$ are computed. For this class, these are the key messages:
