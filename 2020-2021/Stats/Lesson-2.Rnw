...
This work is Copyright, \copyright, 2014-2020 Ramon Diaz-Uriarte, and is licensed under a \textbf{Creative Commons
...
This is section is provided so that you can refer to several different
ways of reshaping data, but we will skip the details in class, unless you
run intro problems. In class we will definitely cover section
\ref{datashape} and read section \ref{reshapefinal}.
...
additional information if we need to. Whole papers have been written about
these issues\footnote{For example, Wickham's ``Tidy data'', in the
