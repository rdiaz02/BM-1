...
Note, however, that this section should not really be necessary after
previous sections.


So that everything is in the open, I will simulate the data here, but in
these notes I won't provide details about how/why the data have been
simulated that way. We will pretend there is a response variable, called
``Y'', two groups (``g1'' and ``g2'') and another variable, called
``Z''. Z might be some reporter protein, or something that can be taken as
a proxy for cell volume, etc. 
...
use). This is more advanced material, though:
