...
forum, refer to the section numbers which change less than the page numbers.
\clearpage
...
organize your data in a table structure like:
...
\begin{tabular}{ccc}
  ...
  \ldots&\ldots&\ldots\\
\end{tabular}
\caption{Paired data in a ``unstacked'' shape/format.}
\end{table}


That looks ok, but is really a very limiting format for the data. Think
about what would you do if you had additional ``tumor'' ``non-tumor'' data
per subject, or you had additional covariates for each measure, etc. That
is why we often want to have the data in a ``stacked'' format as:
...
\begin{tabular}{ccc}
  ...
  \ldots&\ldots&\ldots\\
\end{tabular}
\caption{Paired data in a ``stacked'' shape/format.}
\end{table}

This is arguably a much more useful way of storing the data for most
general analyses and we can keep adding additional information. Whole
papers have been written about these issues\footnote{For example,
  Wickham's ``Tidy data'', in the \textit{Journal of Statistical
    Software}: \Burl{http://www.jstatsoft.org/v59/i10}}, but we will stop here.

\subsubsection{Reshaping our data}
...
the command line. Many, many, many\footnote{For instance:
  ...
  heavily biased toward the ``Hadely's way''.}. And yes, it is unavoidable
...
\item Create a data set that contains only the values for observations
  ...
  \verb@Condition == ``Cancer''@. Call this data set cc.
...
here fully specifying all the needed arguments:
...
OK, so we will do it in a third way. You need to install the
\CRANpkg{tidyr} package. If you don't have it, install it. This is not a
big deal: it is just to show you a third way.
...
I'll use \texttt{merged2}; you can use \texttt{merged} or \texttt{merged3}
or or \texttt{merged4}
...
t.test(merged2$Cancer, merged2$NC, alternative='two.sided', conf.level=.95, 
  paired=TRUE)
@ 
...
% FIXME this is Cancer - NC, but I keep referring to it the other way
% around in the reshaping, later below And check consistency with \ref{ptr}


%% FIXME: ditto: this is the old m3
...
xx: eh, we can use diffs before. Leave it there, and reuse here
<<>>=
diffs <- dmyc$myc[1:12] - dmyc$myc[13:24]
wilcox.test(diffs)
@ 
...
%% reshape in Rcmdr?
%% paired tests in Rcmdr?


%% could use unstack, from the command line, after sorting
