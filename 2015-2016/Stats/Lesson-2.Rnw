...
is why we often want to have the data in a ``stacked'' format as in Table \ref{stacked}:
...
the command line. Many, many, many\footnote{For instance:
  ...
  heavily biased towards the ``Hadely's way''.}. And yes, it is unavoidable
...
here fully specifying all the needed arguments\footnote{In this case, you
...
<<>>=
with(merged3, (t.test(diff.nc.c, alternative='two.sided', mu=0.0, conf.level=.95)))
@ 
...
(section \label{paired-single}), so we will reuse them: 
