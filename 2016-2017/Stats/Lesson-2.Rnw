...
This work is Copyright, \copyright, 2014, 2015, 2016, Ramon Diaz-Uriarte, and is
...
Now load the ``plotByGroup'' plugin \footnote{You don't have it installed?
  Install it, then. You should know how to do it by now.} and you'll find
a new menu entry, under ``Graphs'', that says ``Plot by group''. Use
...
%%% en clase, decir xq p-value es type I error rate e inversión de CI
